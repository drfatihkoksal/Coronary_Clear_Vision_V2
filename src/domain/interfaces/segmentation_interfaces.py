...
class IPostProcessor(Protocol):
    ...
    def process(self, mask: np.ndarray, options: Dict[str, Any]) -> np.ndarray:
        """
    ...
    def smooth_boundaries(self, mask: np.ndarray, iterations: int) -> np.ndarray:
        """
        Sınırları yumuşatır.
...
class ISegmentationVisualization(Protocol):
    ...
    def visualize_segmentation(
        self, image: np.ndarray, mask: np.ndarray, features: Optional[VesselFeatures] = None
    ) -> np.ndarray:
        """
...
class ISegmentationPersistence(Protocol):
    ...
    def save_segmentation(
        self, result: SegmentationResult, file_path: str, format: str = "npy"
    ) -> bool:
        """
    ...
    def export_mask(self, mask: np.ndarray, file_path: str, format: str = "png") -> bool:
        """
