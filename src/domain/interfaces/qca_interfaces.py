...
class IDiameterCalculator(Protocol):
    ...
    def calculate_diameters(
        self, edge_points: np.ndarray, centerline: np.ndarray, calibration: CalibrationData
...
class IEdgeDetector(Protocol):
    ...
    def detect_edges(self, image: np.ndarray, initial_contour: np.ndarray, **kwargs) -> np.ndarray:
        """
...
class IStenosisDetector(Protocol):
    ...
    def detect_stenosis(
        self, measurements: List[VesselMeasurement], threshold_percent: float = 50.0
    ) -> Optional[StenosisData]:
        """
...
class ICalibrationService(ABC):
    ...
    def calibrate_manual(
        self, points: List[Tuple[float, float]], known_length_mm: float
    ) -> CalibrationData:
        """
    ...
    @abstractmethod
    def calibrate_catheter(self, catheter_pixels: float, catheter_size_fr: int) -> CalibrationData:
        """
...
class IQCAAnalysisService(ABC):
    ...
    def analyze_sequential(
        self, requests: List[QCAAnalysisRequest], progress_callback=None
    ) -> dict:
        """
...
class IQCAExportService(Protocol):
    ...
    def export_to_excel(self, results: QCAAnalysisResult, file_path: str) -> bool:
        """Excel'e aktar"""
        ...

    def export_to_csv(self, results: QCAAnalysisResult, file_path: str) -> bool:
        """CSV'ye aktar"""
        ...

    def export_to_pdf(
        self, results: QCAAnalysisResult, file_path: str, include_images: bool = True
    ) -> bool:
        """PDF raporu oluştur"""
        ...
...
class IQCAVisualizationService(Protocol):
    ...
    def create_diameter_graph(self, measurements: List[VesselMeasurement]) -> Any:
        """Çap grafiği oluştur"""
        ...

    def create_vessel_overlay(
        self, image: np.ndarray, centerline: np.ndarray, edges: np.ndarray
    ) -> np.ndarray:
        """Damar overlay'i oluştur"""
        ...

    def create_stenosis_visualization(
        self, image: np.ndarray, stenosis: StenosisData
    ) -> np.ndarray:
