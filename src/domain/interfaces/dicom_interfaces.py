...
from typing import Protocol, List, Optional, Dict, Any, Tuple, Union
from pathlib import Path
import numpy as np

from src.domain.models.dicom_models import (
    DicomStudy, DicomSeries, DicomFrame, DicomLoadRequest,
    DicomLoadResult, DicomWindowLevel, DicomProjectionInfo,
    ProjectionType, DicomModality
)
...
class IDicomFrameExtractor(Protocol):
    ...
    def extract_frame(self, 
                     series: DicomSeries,
                     frame_index: int,
                     apply_window_level: bool = True) -> Optional[np.ndarray]:
        """
        Frame çıkar.
    ...
    def extract_all_frames(self,
                          series: DicomSeries,
                          apply_window_level: bool = True) -> List[np.ndarray]:
        """
...
class IDicomAnonymizer(Protocol):
    ...
    def anonymize_study(self,
                       study: DicomStudy,
                       preserve_dates: bool = False) -> DicomStudy:
        """
...
class IDicomPixelProcessor(Protocol):
    ...
    def normalize_pixels(self,
                        ...
                        output_range: Tuple[float, float] = (0.0, 1.0)) -> np.ndarray:
        """
...
class IDicomValidator(Protocol):
    ...
    def validate_required_tags(self,
                              file_path: Path,
                              required_tags: List[str]) -> Tuple[bool, List[str]]:
        """
