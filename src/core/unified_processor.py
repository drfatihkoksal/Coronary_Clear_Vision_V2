...
class ProcessorResult:
    ...
    def __init__(self, frame_index: int, success: bool, data: Dict[str, Any], error: Optional[str] = None):
...
class BaseProcessor(QThread):
    ...
    def process_frame(self, frame_index: int, frame_data: np.ndarray, 
                     progress_callback: Optional[Callable] = None) -> ProcessorResult:
        """
    ...
    def run(self):
        ...
        try:
            for i, (frame_index, frame_data) in enumerate(self.frames):
                ...
                # Update progress
                progress_msg = f"Processing frame {frame_index + 1}" if not self.is_single_frame else "Processing..."
            ...
            if not self._stop_requested:
                logger.info(f"Processing completed: {successful_count}/{total_frames} frames successful")
...
class UnifiedSegmentationProcessor(BaseProcessor):
    ...
    def __init__(self, frames: List[Tuple[int, np.ndarray]], 
    ...
    def process_frame(self, frame_index: int, frame_data: np.ndarray,
        ...
        if len(points) < 2:
            return ProcessorResult(
                ...
                False, 
                {}, 
                f"Insufficient points for frame {frame_index}"
            )
...
class UnifiedQCAProcessor(BaseProcessor):
    ...
    def __init__(self, frames: List[Tuple[int, Dict]], 
                 qca_analyzer,
    ...
    def process_frame(self, frame_index: int, segmentation_result: Dict,
                     progress_callback: Optional[Callable] = None) -> ProcessorResult:
        ...
        if result.get('success'):
            ...
            return ProcessorResult(
                ...
                False,
                {},
                result.get('error', 'QCA analysis failed')
            )


def create_segmentation_processor(dicom_parser, viewer_widget, frame_indices: List[int],
                                 segmentation_service, single_frame: bool = False) -> UnifiedSegmentationProcessor:
    """
...
def create_qca_processor(segmentation_results: Dict[int, Dict], 
                        qca_analyzer,
                        calibration_factor: Optional[float] = None,
                        single_frame: bool = False) -> UnifiedQCAProcessor:
    """
