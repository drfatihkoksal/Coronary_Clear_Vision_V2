...
class SequentialSegmentationQCAProcessor(QThread):
    ...
    def __init__(
        ...
        dicom_parser,
        viewer_widget,
        tracked_frames: Optional[List[int]] = None,
        ...
        if single_frame_data:
            ...
            self.single_frame_points = {
                single_frame_data["frame_index"]: single_frame_data["points"]
            }
        ...
        if main_window and hasattr(main_window, "segmentation_model"):
            self.segmentation_service.model_manager._segmentation_model = (
                main_window.segmentation_model
            )
    ...
    def run(self):
        ...
        logger.warning(
            f"🚀 [TRACKED DEBUG] SEQUENTIAL PROCESSOR STARTING with {len(self.tracked_frames)} frames!"
        )
        ...
        try:
            for i, frame_idx in enumerate(self.tracked_frames):
                ...
                try:
                    ...
                    print(
                        f"🔍 [TRACKED DEBUG] Frame {frame_idx}: Retrieved {len(tracking_points)} tracking points: {tracking_points}"
                    )
                    logger.warning(
                        f"🔍 [TRACKED DEBUG] Frame {frame_idx}: Retrieved {len(tracking_points)} tracking points: {tracking_points}"
                    )
                    ...
                    logger.info(
                        f"Frame {frame_idx}: Starting segmentation with {len(tracking_points)} points"
                    )
                    ...
                    if not seg_result.get("success"):
                        raise ValueError(
                            f"Segmentation failed: {seg_result.get('error', 'Unknown error')}"
                        )
                    ...
                    if self.calibration_factor:
                        ...
                        tracked_points_for_centerline = (
                            tracking_points if len(tracking_points) >= 2 else None
                        )
                        use_tracked_centerline = tracked_points_for_centerline is not None

                        print(
                            f"🎯 [TRACKED DEBUG] Frame {frame_idx}: tracked_points_for_centerline = {tracked_points_for_centerline}"
                        )
                        print(
                            f"🎯 [TRACKED DEBUG] Frame {frame_idx}: use_tracked_centerline = {use_tracked_centerline}"
                        )
                        logger.warning(
                            f"🎯 [TRACKED DEBUG] Frame {frame_idx}: use_tracked_centerline = {use_tracked_centerline}"
                        )
                        ...
                        if use_tracked_centerline:
                            print(
                                f"✅ Frame {frame_idx}: Using tracked centerline with {len(tracked_points_for_centerline)} points"
                            )
                            logger.warning(
                                f"✅ Frame {frame_idx}: Using tracked centerline with {len(tracked_points_for_centerline)} points"
                            )
                            ...
                            print(
                                f"❌ Frame {frame_idx}: Using AngioPy centerline (insufficient tracked points: {len(tracking_points)})"
                            )
                            logger.warning(
                                f"❌ Frame {frame_idx}: Using AngioPy centerline (insufficient tracked points: {len(tracking_points)})"
                            )
                        ...
                        use_gradient_method = (
                            False  # Use segmentation mask for diameter measurement
                        )
                        gradient_method = (
                            "second_derivative"  # Not used when use_gradient_method=False
                        )
                        ...
                        if qca_result.get("success"):
                            ...
                            if hasattr(self, "main_window") and self.main_window:
                                if hasattr(self.main_window, "get_cardiac_phase_for_frame"):
                                    phase_info = self.main_window.get_cardiac_phase_for_frame(
                                        frame_idx
                                    )
                                    if phase_info:
                            ...
                            logger.warning(
                                f"Frame {frame_idx}: QCA failed - {qca_result.get('error')}"
                            )
                    else:
            ...
            logger.info(
                f"Sequential processing completed: {len(self.segmentation_results)} segmentations, "
...
def create_sequential_processor(
    ...
    calibration_factor: Optional[float] = None,
    use_curvature_resistant_centerline: bool = False,
) -> Optional[SequentialSegmentationQCAProcessor]:
    """
    ...
    if hasattr(main_window.viewer_widget, "overlay_item") and hasattr(
        main_window.viewer_widget.overlay_item, "frame_points"
    ):
    ...
    if calibration_factor is None:
        ...
        if calibration_factor is None and hasattr(main_window, "dicom_parser"):
            ...
            if calibration_factor:
                logger.info(
                    f"Using DICOM pixel spacing as calibration: {calibration_factor:.5f} mm/pixel"
                )
...
def create_single_frame_processor(
    main_window,
    frame_index: int,
    points: List[Tuple[int, int]],
    calibration_factor: Optional[float] = None,
    use_curvature_resistant_centerline: bool = False,
) -> SequentialSegmentationQCAProcessor:
    """
    ...
    single_frame_data = {"frame_index": frame_index, "points": points}
