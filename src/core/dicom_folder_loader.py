            ...
            for series_uid, files in series_dict.items():
                if files:
                    ...
                    has_pixel_data = any([
                        ...
                        hasattr(first_dcm, 'NumberOfFrames') and int(first_dcm.NumberOfFrames) > 0
                    ])
                    ...
                    if not has_pixel_data or any(sop_class.startswith(prefix) for prefix in non_image_sop_classes):
                        logger.info(f"Skipping non-image series: {series_uid} - {first_dcm.get('SeriesDescription', 'Unknown')}")
                        continue
                    ...
                    if hasattr(first_dcm, 'PositionerPrimaryAngle'):
                        series_info.primary_angle = float(first_dcm.PositionerPrimaryAngle)
                    if hasattr(first_dcm, 'PositionerSecondaryAngle'):
