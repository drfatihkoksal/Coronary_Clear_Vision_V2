...
class OverlayItem(QGraphicsItem):
    ...
    def paint(self, painter: QPainter, option, widget):
        """Paint all overlays"""
        logger.debug(
            f"OverlayItem.paint called - show_qca: {self.show_qca}, has_qca_results: {self.qca_results is not None}"
        )
        ...
        if (
            hasattr(self, "calibration_centerline")
            and self.calibration_centerline is not None
            and hasattr(self, "show_calibration_centerline")
            and self.show_calibration_centerline
        ):
            self.paint_calibration_centerline(painter)

        # Draw automatic calibration diameter measurements if available and visible
        if (
            hasattr(self, "calibration_diameters")
            and self.calibration_diameters is not None
            and hasattr(self, "show_calibration_diameters")
            and self.show_calibration_diameters
        ):
        ...
        if (
            self.show_points
            and self.current_frame_index in self.frame_points
            and self.frame_points[self.current_frame_index]
        ):
    ...
    def paint_segmentation(self, painter: QPainter):
        ...
        if not isinstance(color_name, str):
            logger.warning(
                f"Invalid color type: {type(color_name)}, value: {color_name}. Using default 'Red'"
            )
        ...
        if contour_only:
            ...
            if np.any(vessel_mask):
                contours, _ = cv2.findContours(
                    vessel_mask.astype(np.uint8), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
                )
            ...
            if np.any(reference_mask):
                contours, _ = cv2.findContours(
                    reference_mask.astype(np.uint8), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
                )
            ...
            if len(unique_values) > 2:  # More than just 0 and 255/1
                ...
                overlay[self.segmentation_mask > 0] = [
                    color.red(),
                    color.green(),
                    color.blue(),
                    int(255 * opacity),
    ...
    def paint_qca(self, painter: QPainter):
        ...
        logger.info(
            f"paint_qca called - results: {results is not None}, success: {results.get('success') if results else False}"
        )
        ...
        if (
            settings.get("show_centerline", True)
            and "centerline" in results
            and results["centerline"] is not None
        ):
            ...
            if len(centerline) > 1:
                ...
                for i in range(
                    0, len(centerline), max(1, len(centerline) // 20)
                ):  # Draw every Nth point
                    point = centerline[i]
                    painter.drawEllipse(QPointF(point[1], point[0]), 2, 2)
        else:
            logger.warning(
                f"Centerline not drawn - show_centerline: {settings.get('show_centerline', True)}, has_centerline: {'centerline' in results}"
            )

        # Debug log for diameter data
        if "diameters_pixels" in results:
            logger.info(
                f"QCA Results has diameters_pixels: {len(results['diameters_pixels'])} values"
            )
        else:
        ...
        if (
            settings.get("show_diameter", False)
            and "centerline" in results
            and results["centerline"] is not None
        ):
            centerline = results["centerline"]
            ...
            diameters = (
                results.get("diameters")
                or results.get("diameters_px")
                or results.get("diameters_pixels")
            )

            logger.info(
                f"Diameter visualization - show_diameter={settings.get('show_diameter')}, centerline_length={len(centerline)}, has_diameters={diameters is not None}"
            )
            ...
            logger.info(
                f"Diameter visualization: centerline={len(centerline)} points, diameters={len(diameters)} values, interval={line_sample_interval}"
            )
            ...
            for i in range(0, num_points, line_sample_interval):
                ...
                if i > 0 and i < len(centerline) - 1:
                    ...
                    if norm > 0:
                        ...
                        if (
                            "left_edges" in results
                            and "right_edges" in results
                            and "perpendiculars" in results
                            and i < len(results["left_edges"])
                        ):
                        ...
                        p1 = QPointF(center_x - perp_x * left_edge, center_y - perp_y * left_edge)
                        p2 = QPointF(center_x + perp_x * right_edge, center_y + perp_y * right_edge)
                        painter.drawLine(p1, p2)
                        drawn_count += 1

            logger.info(
                f"Drew {drawn_count} diameter lines out of {num_points // line_sample_interval} expected"
            )
    ...
    def paint_calibration_mask(self, painter: QPainter):
        ...
        contours, _ = cv2.findContours(
            self.calibration_mask.astype(np.uint8), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
        )
    ...
    def paint_calibration_diameters(self, painter: QPainter):
        ...
        if (
            not hasattr(self, "calibration_centerline")
            or self.calibration_centerline is None
            or not hasattr(self, "calibration_diameters")
            or self.calibration_diameters is None
            or not hasattr(self, "calibration_left_edges")
            or self.calibration_left_edges is None
            or not hasattr(self, "calibration_right_edges")
            or self.calibration_right_edges is None
            or not hasattr(self, "calibration_perpendiculars")
            or self.calibration_perpendiculars is None
        ):
            return
        ...
        for i in range(0, len(centerline), sample_interval):
            ...
            if i < 3:
                ...
                logger.info(
                    f"  - Line {i}: center=({cx:.1f},{cy:.1f}), diameter={diameter:.1f}, "
...
class EnhancedDicomViewer(QGraphicsView):
    ...
    def __init__(self):
        ...
        self.heartbeat_proxy.setFlag(
            QGraphicsItem.GraphicsItemFlag.ItemIgnoresTransformations, True
        )  # Don't scale with scene
        self.heartbeat_proxy.setAcceptedMouseButtons(
    ...
    def mousePressEvent(self, event: QMouseEvent):
        ...
        is_tracking_in_view = self.tracking_enabled and self.mode_manager.is_in_mode(
            ViewerMode.VIEW
        )
    ...
    def _handle_point_placement(self, dicom_x: int, dicom_y: int, event: QMouseEvent):
        ...
        if current_mode == ViewerMode.VIEW and self.tracking_enabled:
            ...
            logger.info(
                f"Vessel centering in tracking mode: ({dicom_x}, {dicom_y}) -> ({centered_x}, {centered_y})"
            )
        ...
        # Apply catheter centering for calibration mode
            ...
            logger.info(
                f"Catheter centering in calibration mode: ({dicom_x}, {dicom_y}) -> ({centered_x}, {centered_y})"
            )
            dicom_x, dicom_y = centered_x, centered_y
    ...
    def display_frame(self, frame_index: int):
        ...
        self.current_frame = frame.copy() if frame is not None else None  # Store raw frame copy
        ...
        if (
            ...
            and hasattr(self, "_prev_frame_index")
                    ...
                    if prev_points and frame_index not in self.overlay_item.frame_points:
                        ...
                        if prev_frame is not None:
                            ...
                            if result and frame_index in self.overlay_item.frame_points:
                                self.overlay_item.user_points = self.overlay_item.frame_points[
                                    frame_index
                                ].copy()
    ...
    def mousePressEvent(self, event: QMouseEvent):
        ...
        if self.multi_frame_mode and False:
            ...
            and self.interaction_mode == "view"
        ):
            ...
            if len(self.calibration_points) == 2:
                ...
                self.calibration_points_selected.emit(
                    self.calibration_points[0], self.calibration_points[1]
                )
                ...
                if hasattr(self, "catheter_diameter_mm") and self.current_frame is not None:
                    ...
                    if width_pixels > 0:
                        ...
                        logger.info(
                            f"Catheter size: {getattr(self, 'catheter_size', 'Unknown')} = {self.catheter_diameter_mm}mm"
                        )
        ...
        if not (self.segmentation_mode and event.button() == Qt.MouseButton.LeftButton):
            if not (
                self.interaction_mode in ["segment", "calibrate"]
                and event.button() == Qt.MouseButton.LeftButton
            ):
                if not (
                    event.button() == Qt.MouseButton.RightButton and self.adjusting_window_level
                ):
                    super().mousePressEvent(event)
        ...
        if self.current_frame is not None:
            ...
            if (
                (
                    ...
                    or (self.tracking_enabled and self.interaction_mode == "view")
                )
    ...
    def mouseReleaseEvent(self, event: QMouseEvent):
        ...
        ) and self.adjusting_window_level:
            self.adjusting_window_level = False
            # Restore cursor
    ...
    def track_points_to_frame(self, new_frame_index: int):
        ...
        logger.debug(
            f"track_points_to_frame called: {self.current_frame_index} -> {new_frame_index}"
        )
        ...
        try:
            ...
            # Validate frames
            logger.debug(
                f"Current frame shape: {self.current_frame.shape if self.current_frame is not None else 'None'}"
            )
            ...
            if tracking_results:
                ...
                for i, point in enumerate(current_points):
                    ...
                    if point_id and point_id in tracking_results:
                        ...
                        logger.warning(
                            f"Failed to track point {i} (ID: {point_id}) at frame {new_frame_index}"
                        )
                ...
                logger.warning(
                    f"No tracking results from frame {self.current_frame_index} to frame {new_frame_index}, copying points"
                )
    ...
    def _update_tracked_points(self, frame_index: int):
        ...
        frame = (
            self.dicom_parser.get_frame(frame_index)
        ...
        for i, point in enumerate(frame_points):
            ...
            success = self.point_tracker.add_point(
                point_id, Point(float(point[0]), float(point[1]))
            )
            if success:
    ...
    def track_all_frames(self, progress_callback=None, start_frame=None, end_frame=None):
        ...
        if self.current_frame_index < start_frame or self.current_frame_index > end_frame:
            logger.warning(
                f"Current frame {self.current_frame_index} is outside range {start_frame}-{end_frame}"
            )
            return False
        ...
        for frame_idx in range(
            self.current_frame_index + 1, min(end_frame + 1, self.dicom_parser.num_frames)
        ):
        ...
        for frame_idx in range(self.current_frame_index - 1, max(start_frame - 1, -1), -1):
            if progress_callback:
                ...
                if total_backward > 0:
                    progress = 50 + int(
                        ((self.current_frame_index - frame_idx) / total_backward) * 50
                    )
                else:
                    progress = 100
                if not progress_callback(progress, 100):
    ...
    def _handle_batch_tracking_edit(self, edited_frame_index: int, edited_point_index: int):
        ...
        msg.setText(
            f"You've edited a point that was tracked through multiple frames.\n\n"
    ...
    def _retrack_from_frame(self, start_frame: int, point_index: int):
        ...
        progress = QProgressDialog(
            "Re-tracking points from edited frame...", "Cancel", 0, 100, self
        )
        ...
        if success:
            ...
            QMessageBox.warning(
                self, "Re-tracking Failed", "Failed to re-track points from the edited frame."
            )
    ...
    def show_point_context_menu(self, global_pos, x: int, y: int):
        ...
        if point_index >= 0:
            ...
            if (
                self.current_frame_index in self.overlay_item.frame_points
                and len(self.overlay_item.frame_points[self.current_frame_index]) > 0
            ):
                menu.addSeparator()
        ...
        start_frame_action.triggered.connect(
            lambda: self.set_frame_range_start(self.current_frame_index)
        )
        menu.addAction(start_frame_action)

        # Set as end frame
        end_frame_action = QAction("🏁 Set as End Frame", self)
        end_frame_action.triggered.connect(
            lambda: self.set_frame_range_end(self.current_frame_index)
        )
        ...
        if self.cardiac_phases is not None and "frame_phases" in self.cardiac_phases:
            ...
            for beat_num in sorted(beat_numbers):
                ...
                beat_action.triggered.connect(
                    lambda checked, b=beat_num: self.select_beat_number(b)
                )
            ...
            beat_around_action.triggered.connect(
                lambda: self.select_beat_around_frame(self.current_frame_index)
            )
            beat_menu.addAction(beat_around_action)

        # Show All Frames / Clear Beat Selection
        if (
            self.frame_range_start is not None
            or self.frame_range_end is not None
            or (self.main_window and hasattr(self.main_window, "navigation_range"))
        ):
            menu.addSeparator()
        ...
        try:
            ...
            if total_weight > 0:
                y_coords, x_coords = np.meshgrid(
                    range(window.shape[0]), range(window.shape[1]), indexing="ij"
                )
                ...
                if dist < window_size / 3:  # Within third of window size
                    ...
                    logger.debug(
                        f"Using weighted centroid for catheter: ({x}, {y}) -> ({final_x}, {final_y})"
                    )
                else:
                    final_x, final_y = extremum_x, extremum_y
                    logger.debug(
                        f"Using extremum for catheter: ({x}, {y}) -> ({final_x}, {final_y})"
                    )
            else:
                final_x, final_y = extremum_x, extremum_y
                logger.debug(
                    f"Using extremum (no valid weights): ({x}, {y}) -> ({final_x}, {final_y})"
                )
    ...
    def find_vessel_center(self, x: int, y: int, window_size: int = 25) -> Tuple[int, int]:
        ...
        try:
            ...
            if total_weight > 0:
                ...
                final_x, final_y = simple_x, simple_y
                logger.debug(
                    f"Using simple minimum (no valid weights): ({x}, {y}) -> ({final_x}, {final_y})"
                )
    ...
    def add_point_at_position(self, x: int, y: int):
        ...
        if (
            hasattr(self, "tracking_enabled")
            and self.tracking_enabled
            and hasattr(self, "mode_manager")
            and self.mode_manager.is_in_mode(ViewerMode.VIEW)
        ):
    ...
    def select_beat_number(self, beat_number: int):
        ...
        logger.info(
            f"Beat {beat_number} selected: frames {start_frame} to {end_frame} (total {len(beat_frames)} frames) [UI: {start_frame + 1} to {end_frame + 1}]"
        )
        ...
        if beat_number == 1:
            ...
            navigate_frame = start_frame
            for phase_info in self.cardiac_phases["frame_phases"]:
                if phase_info.get("beat_number") == beat_number and phase_info.get("phase") == "d2":
    ...
    def clear_segmentation_graphics(self):
        ...
        # Restore original image only if we have a valid frame
        if self.current_frame_index is not None and self.current_frame is not None:
            self.display_frame(self.current_frame_index)
    ...
    def create_segmented_image(self, original_image: np.ndarray, result: dict) -> np.ndarray:
        ...
        if mask is not None:
            ...
            if mask.shape != image.shape[:2]:
                logger.warning(
                    f"Mask shape {mask.shape} != Image shape {image.shape[:2]}, resizing mask"
                )
    ...
    def set_segmentation_overlay(self, result: dict, settings: dict):
        ...
        if result.get("success") and result.get("mask") is not None:
            ...
            if np.sum(mask > 0) > 0:
                ...
                logger.info(
                    f"Mask bounds - Y: [{y_indices.min()}, {y_indices.max()}], X: [{x_indices.min()}, {x_indices.max()}]"
                )
                logger.info(
                    f"Mask area: {(y_indices.max() - y_indices.min()) * (x_indices.max() - x_indices.min())} pixels"
                )
            ...
            if "centerline" in result:
                ...
                logger.info(
                    f"Received centerline with {len(centerline) if centerline is not None else 0} points"
                )
                if centerline is not None and len(centerline) > 0:
                    ...
                    if len(centerline) <= 10:
                        ...
                        logger.info(
                            f"Sample centerline - First: {centerline[0]}, Last: {centerline[-1]}"
                        )
    ...
    def measure_catheter_width(self, p1: tuple, p2: tuple) -> float:
        ...
        logger.debug(
            f"Measuring catheter between ({x1},{y1}) and ({x2},{y2}), line length: {line_length:.1f}px"
        )
        ...
        for i in range(num_samples):
            ...
            for dist in range(-profile_length, profile_length + 1):
                ...
                if (
                    0 <= sample_x < self.current_frame.shape[1]
                    and 0 <= sample_y < self.current_frame.shape[0]
                ):
        ...
        if all_widths:
            ...
            logger.info(
                f"Catheter width measurement: {width:.2f} pixels (from {len(all_widths)} samples)"
            )
            return width
    ...
    def _find_catheter_width_from_profile(
        self, profile: np.ndarray, positions: np.ndarray
    ) -> float:
        """
        ...
        if left_edge is not None and right_edge is not None:
            ...
            logger.debug(
                f"Edge detection: left={left_edge:.1f}, right={right_edge:.1f}, width={width:.1f}"
            )
    ...
    def measure_width_at_point(self, edges: np.ndarray, point: tuple, direction: tuple) -> float:
        ...
        for sign in [-1, 1]:
            for dist in range(1, max_dist):
                ...
                # Check bounds
                if 0 <= sample_x < edges.shape[1] and 0 <= sample_y < edges.shape[0]:
    ...
    def _apply_vessel_centering_to_frame(
        self, frame: np.ndarray, x: int, y: int, window_size: int = 25
    ) -> Tuple[int, int]:
        """
    ...
    def _track_points_between_frames(
        self, prev_frame: np.ndarray, curr_frame: np.ndarray, points: list
    ) -> list:
        ...
        for i, point in enumerate(points):
            temp_tracker.add_point(str(i), Point(point[0], point[1]))
    ...
    def perform_single_point_tracking(self, point_x: float, point_y: float):
        ...
        try:
            ...
            proximal_segment, distal_segment = self._create_proximal_distal_segments(
                aligned_centerline
            )
    ...
    def _extract_centerline_from_segmentation(self, segmentation_mask):
        ...
        try:
            ...
            if segmentation_mask.dtype != bool:
                binary_mask = (
                    segmentation_mask > 0.5
                    if segmentation_mask.max() <= 1.0
                    else segmentation_mask > 127
                )
            else:
    ...
    def set_calibration_mask_overlay(self, mask: np.ndarray):
        ...
        if hasattr(self, "overlay_item") and self.overlay_item:
            ...
            self._request_update()
            logger.info(
                f"✅ Set calibration mask overlay - shape: {mask.shape}, nonzero: {np.count_nonzero(mask)}"
            )
        else:
    ...
    def set_calibration_diameter_overlay(self, centerline: np.ndarray, diameters: list):
        ...
        if hasattr(self, "overlay_item") and self.overlay_item:
            ...
            self._request_update()
            logger.info(
                f"✅ Set calibration diameter overlay: {len(centerline)} points, {len(diameters)} diameters"
            )
        else:
