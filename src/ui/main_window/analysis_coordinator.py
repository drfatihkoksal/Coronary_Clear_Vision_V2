...
class AnalysisCoordinator(QObject):
    ...
    def start_calibration(self):
        ...
        try:
            ...
            if not self.main_window.dicom_parser:
                QMessageBox.warning(
                    self.main_window, "No DICOM Loaded", "Please load a DICOM file first."
                )
                return
    ...
    def start_qca_analysis(self, segmentation_result: dict = None):
        ...
        try:
            ...
            # Check calibration
            if (
                not hasattr(self.main_window, "calibration_factor")
                or not self.main_window.calibration_factor
            ):
                QMessageBox.warning(
                    self.main_window,
    ...
    def start_sequential_processing(self, start_frame: int = None, end_frame: int = None):
        ...
        try:
            ...
            if start_frame >= end_frame:
                QMessageBox.warning(
                    self.main_window, "Invalid Range", "End frame must be greater than start frame."
                )
                return
            ...
            self.sequential_processor.segmentation_completed.connect(
                self._on_sequential_segmentation
            )
    ...
    def start_tracking_workflow(self, start_frame: int = None, end_frame: int = None):
        ...
        try:
            ...
            if (
                not hasattr(self.main_window.viewer_widget, "overlay_item")
                or not self.main_window.viewer_widget.overlay_item.frame_points
            ):
                QMessageBox.warning(
                    self.main_window, "No Points to Track", "Please add some tracking points first."
                )
                return
    ...
    def _on_sequential_all_completed(self, seg_results: dict, qca_results: dict):
        ...
        self.analysis_completed.emit(
            "sequential", {"segmentation_results": seg_results, "qca_results": qca_results}
        )
