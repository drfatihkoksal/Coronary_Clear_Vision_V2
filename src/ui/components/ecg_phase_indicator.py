...
from src.domain.models.ecg_models import CardiacPhase, PhaseTransition
...
class PhaseIndicatorWidget(QWidget):
    ...
    def _draw_phase_segments(self, painter: QPainter, cx: int, cy: int, radius: int):
        ...
        for i, phase in enumerate(phases):
            ...
            # Pasta dilimi çiz
            painter.drawPie(cx - radius, cy - radius,
                          radius * 2, radius * 2,
    ...
    def _draw_phase_labels(self, painter: QPainter, cx: int, cy: int, radius: int):
        ...
        for phase, (code, name) in labels.items():
            ...
            painter.drawText(pos_x - 20, pos_y - 10, 40, 20,
                           Qt.AlignmentFlag.AlignCenter, code)
            
            # İsim (küçük font)
            small_font = QFont("Arial", 9)
            painter.setFont(small_font)
            painter.setPen(QPen(QColor(100, 100, 100)))
            painter.drawText(pos_x - 40, pos_y + 5, 80, 20,
    ...
    def _draw_current_phase_indicator(self, painter: QPainter, cx: int, cy: int, radius: int):
        ...
        painter.drawEllipse(cx - radius - 5, cy - radius - 5,
...
class ECGPhaseIndicator(QWidget):
    ...
    def set_phase(self, phase: CardiacPhase, cycle_number: int = 0,
                  heart_rate: float = 0.0, progress: float = 0.5):
        """
        Kardiyak fazı ayarla.
    ...
    def _update_labels(self):
        ...
        if self._current_phase:
            ...
            # Faz rengi
            color = self.phase_widget._phase_colors.get(
                self._current_phase, QColor(0, 0, 0)
            )
