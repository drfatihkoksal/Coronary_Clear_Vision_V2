...
from src.domain.models.qca_models import QCAAnalysisResult, StenosisGrade
...
class QCAResultsDisplay(QWidget):
    ...
    def _create_measurements_widget(self) -> QWidget:
        ...
        # Seçim sinyali
        self.measurements_table.itemSelectionChanged.connect(self._on_measurement_selection_changed)
    ...
    def _update_stenosis_info(self, result: QCAAnalysisResult):
        ...
        self.stenosis_percent_label.setText(f"Diameter Stenosis: {stenosis.percent_diameter:.1f}%")
        self.stenosis_area_label.setText(f"Area Stenosis: {stenosis.percent_area:.1f}%")
        self.stenosis_location_label.setText(f"Location: {stenosis.location_mm:.1f} mm")
        self.stenosis_length_label.setText(f"Length: {stenosis.length_mm:.1f} mm")
        self.stenosis_grade_label.setText(f"Clinical Grade: {stenosis.grade.value.upper()}")
    ...
    def clear_results(self):
        ...
        # Metrikleri sıfırla
        for label in [
            self.mean_diameter_label,
            self.min_diameter_label,
            self.max_diameter_label,
            self.vessel_type_label,
            self.analysis_time_label,
            self.calib_factor_label,
            self.calib_method_label,
            self.calib_confidence_label,
        ]:
