...
from src.domain.models.tracking_models import (
    TrackingParameters, TrackingMethod, MotionModel,
    TrackedPoint, TrackingStatus
)
...
class TrackingControlPanel(QWidget):
    ...
    def _connect_signals(self):
        ...
        # Tablo seçimi
        self.points_table.selectionModel().selectionChanged.connect(
    ...
    def _update_statistics(self):
        ...
        active = sum(1 for p in self._tracked_points.values() 
                    if p.status == TrackingStatus.ACTIVE)
        lost = sum(1 for p in self._tracked_points.values() 
                  if p.status == TrackingStatus.LOST)
