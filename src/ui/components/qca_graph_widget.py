...
from src.domain.models.qca_models import (
    QCAAnalysisResult, VesselMeasurement, StenosisData, QCASequentialResult
)
...
class QCAGraphWidget(QWidget):
    ...
    def _draw_diameter_profile(self):
        ...
        ax.axhline(y=mean_diameter, color='green', linestyle='--', 
                  alpha=0.7, label=f'Mean: {mean_diameter:.2f} mm')
        
        # Stenoz bölgesi varsa işaretle
        if self._current_result.stenosis_data:
            stenosis = self._current_result.stenosis_data
            ax.axvspan(stenosis.location_mm - stenosis.length_mm/2,
                      stenosis.location_mm + stenosis.length_mm/2,
                      alpha=0.3, color='red', label='Stenosis Region')
            
            # MLD işareti
            ax.axhline(y=stenosis.minimal_lumen_diameter_mm, 
                      color='red', linestyle=':',
    ...
    def _draw_area_profile(self):
        ...
        # Alan profili
        ax.fill_between(positions, 0, areas, alpha=0.5, color='skyblue', label='Cross-sectional Area')
        ax.plot(positions, areas, 'b-', linewidth=2)
        
        # Ortalama alan
        mean_area = np.mean(areas)
        ax.axhline(y=mean_area, color='green', linestyle='--',
                  alpha=0.7, label=f'Mean: {mean_area:.2f} mm²')
    ...
    def _draw_temporal_diameter(self):
        ...
        # Ana çap eğrisi
        ax.plot(frames, diameters, 'b-', linewidth=2, marker='o', 
               markersize=5, label='Mean Diameter')
        
        # İstatistikler
        mean_diam = np.mean(diameters)
        std_diam = np.std(diameters)
        
        ax.axhline(y=mean_diam, color='green', linestyle='--',
                  label=f'Mean: {mean_diam:.2f} mm')
        ax.fill_between(frames, mean_diam - std_diam, mean_diam + std_diam,
        ...
        if 'pulsatility_index' in self._sequential_result.temporal_analysis:
            ...
            ax.text(0.02, 0.98, f'Pulsatility Index: {pi:.3f}',
    ...
    def _draw_confidence_plot(self):
        ...
        # Güven değerleri
        ax.plot(positions, confidences, 'g-', linewidth=2, marker='s',
