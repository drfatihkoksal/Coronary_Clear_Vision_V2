...
class TrackingResultsDisplay(QWidget):
    ...
    def _create_instant_results_tab(self) -> QWidget:
        ...
        self.instant_table.setHorizontalHeaderLabels(["Nokta", "X", "Y", "Hız", "Güven", "Durum"])
    ...
    def _update_trajectory_display(self):
        ...
        for row, point in enumerate(points):
            ...
            self.trajectory_table.setItem(
                row, 1, QTableWidgetItem(f"{stats['total_distance']:.1f} px")
            )
            self.trajectory_table.setItem(
                row, 2, QTableWidgetItem(f"{stats['avg_speed']:.1f} px/f")
            )
            self.trajectory_table.setItem(
                row, 3, QTableWidgetItem(f"{stats['max_speed']:.1f} px/f")
            )
    ...
    def _update_performance_display(self):
        ...
        if self._current_result:
            ...
            self.performance_text.append(
                f"İzlenen Nokta: {len(self._current_result.tracked_points)}"
            )
            self.performance_text.append(
                f"Başarısız Nokta: {len(self._current_result.failed_points)}"
            )
    ...
    def _calculate_trajectory_stats(self, trajectory: List[Point2D]) -> Dict:
        ...
        if len(trajectory) >= 3:
            for i in range(2, len(trajectory)):
                v1 = (
                    trajectory[i - 1].x - trajectory[i - 2].x,
                    trajectory[i - 1].y - trajectory[i - 2].y,
                )
                v2 = (trajectory[i].x - trajectory[i - 1].x, trajectory[i].y - trajectory[i - 1].y)
