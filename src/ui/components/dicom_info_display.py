...
from src.domain.models.dicom_models import (
    ...
    DicomPatientInfo, DicomStudyInfo, DicomSeriesInfo,
    DicomImageInfo, DicomProjectionInfo, DicomPixelSpacing
)
...
class DicomInfoDisplay(QWidget):
    ...
    def _update_technical_info(self):
        ...
        if self._current_series.projection_info:
            ...
            if proj.distance_source_to_detector is not None:
                proj_params.append(("Kaynak-Detektör", f"{proj.distance_source_to_detector:.1f} mm"))
    ...
    def _update_tags_tree(self):
        ...
        if self._current_series:
            ...
            patient_item.addChild(QTreeWidgetItem(["(0010,0010) PatientName", "PN", 
                                                 self._current_study.patient_info.patient_name if self._current_study else ""]))
            patient_item.addChild(QTreeWidgetItem(["(0010,0020) PatientID", "LO",
            ...
            if self._current_study:
                study_item.addChild(QTreeWidgetItem(["(0020,000D) StudyInstanceUID", "UI",
            ...
            series_item.addChild(QTreeWidgetItem(["(0020,000E) SeriesInstanceUID", "UI",
                                                 self._current_series.info.series_instance_uid]))
            series_item.addChild(QTreeWidgetItem(["(0008,0060) Modality", "CS",
                                                 self._current_series.info.modality.value]))
