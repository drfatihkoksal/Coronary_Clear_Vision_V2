...
class RWSResultsDisplay(QWidget):
    ...
    def _update_results_table(self, result: RWSAnalysisResult):
        ...
        table_data: List[Tuple[str, str]] = [
            ...
            ("Max RWS Location", f"Index {result.rws_max_location}" if result.rws_max_location >= 0 else "N/A"),
            ("Diameter Change", f"{result.diameter_change_mm:.3f} mm"),
            ("Reference Diameter", f"{result.reference_diameter_mm:.2f} mm"),
            ("End-Diastole Frame", str(result.end_diastole_frame) if result.end_diastole_frame >= 0 else "N/A"),
            ("End-Systole Frame", str(result.end_systole_frame) if result.end_systole_frame >= 0 else "N/A"),
        ]
