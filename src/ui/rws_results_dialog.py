...
from PyQt6.QtWidgets import (
    ...
    QHeaderView, QTextEdit, QDialogButtonBox, QFileDialog,
...
class RWSResultsDialog(QDialog):
    ...
    def create_header_widget(self) -> QWidget:
        ...
        rws_label = QLabel(f"RWS at MLD: {rws_value}%" if rws_value is not None else "RWS at MLD: N/A")
        font = QFont()
        ...
        if rws_value is not None and rws_value > 12:
            rws_label.setStyleSheet("color: #ff4444; background-color: #ffeeee; padding: 10px; border-radius: 5px;")
        elif rws_value is not None:
            rws_label.setStyleSheet("color: #44ff44; background-color: #eeffee; padding: 10px; border-radius: 5px;")
        else:
            rws_label.setStyleSheet("color: #666666; background-color: #f0f0f0; padding: 10px; border-radius: 5px;")
    ...
    def create_summary_tab(self) -> QWidget:
        ...
        mld_data = [
            ...
            ("MLD Variation:", f"{mld_max_val - mld_min_val:.3f} mm" if mld_max_val is not None and mld_min_val is not None else "N/A"),
        ]
        ...
        analysis_data = [
            ...
            ("Calibration Factor:", f"{cal_factor:.4f} mm/pixel" if cal_factor is not None else "N/A"),
            ("Analysis Time:", datetime.fromisoformat(self.results.get('timestamp', datetime.now().isoformat())).strftime("%Y-%m-%d %H:%M:%S")),
        ]
    ...
    def create_frame_detail_widget(self, title: str, frame_idx: Optional[int], 
                                  mld_value: Optional[float], bg_color: str) -> QGroupBox:
            ...
            if 'diameters_mm' in qca_data or 'diameter_profile' in qca_data:
                ...
                if diameters is not None and len(diameters) > 0:
                    ...
                    if mld_idx is not None and 0 <= mld_idx < len(diameters):
                        ...
                        ax.annotate(f'MLD: {diameters[mld_idx]:.3f}mm', 
    ...
    def create_visualizations_tab(self) -> QWidget:
        ...
        if mld_info:
            ...
            if min_frame is not None and min_frame in mld_info:
                ...
                ax1.annotate(f'Min: {mld_values[min_idx]:.2f}mm', 
            ...
            if max_frame is not None and max_frame in mld_info:
                ...
                ax1.annotate(f'Max: {mld_values[max_idx]:.2f}mm', 
            ...
            rws_val = self.results.get("rws_at_mld")
            title_text = f'MLD Variation Across Cardiac Cycle - RWS = {rws_val}%' if rws_val is not None else 'MLD Variation Across Cardiac Cycle - RWS = N/A'
        ...
        if diameter_profiles and min_frame in diameter_profiles and max_frame in diameter_profiles:
            ...
            ax2.plot(min_positions, min_profile, 'r-', linewidth=2, label=f'Min MLD (Frame {min_frame + 1})')
            ax2.plot(max_positions, max_profile, 'g-', linewidth=2, label=f'Max MLD (Frame {max_frame + 1})')
    ...
    def create_detailed_data_tab(self) -> QWidget:
        ...
        detailed_text += f"\nCALIBRATION:\n"
        cal_factor = self.results.get('calibration_factor')
        detailed_text += f"  Factor: {f'{cal_factor:.4f} mm/pixel' if cal_factor is not None else 'N/A'}\n"
