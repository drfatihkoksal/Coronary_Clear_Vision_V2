...
from PyQt6.QtWidgets import (QWidget, QVBoxLayout, QHBoxLayout, QPushButton,
                            QLabel, QGroupBox, QTextEdit, QTabWidget,
...
class RWSAnalysisThread(QThread):
    ...
    def __init__(self, frames, qca_results, cardiac_phases, 
                 frame_timestamps, calibration_factor):
        super().__init__()
        self.frames = frames
...
class EnhancedRWSWidget(QWidget):
    ...
    def create_segment_tab(self) -> QWidget:
        ...
        self.segment_table.setHorizontalHeaderLabels([
            "Segment", "RWS (%)", "Max Diameter (mm)", "Max Frame", 
            "Min Diameter (mm)", "Min Frame"
        ])
    ...
    def start_analysis(self, frames, qca_results, cardiac_phases, 
                      frame_timestamps, calibration_factor):
        ...
        self.analysis_thread = RWSAnalysisThread(
            frames, qca_results, cardiac_phases, 
            frame_timestamps, calibration_factor
        )
    ...
    def update_summary_display(self):
        ...
        self.interpretation_text.setText(
            self.analysis_results['clinical_interpretation']
        )
        ...
        if point_rws_results:
            ...
            measurements.extend([
                ("Proximal RWS", f"{proximal_rws:.1f}%"),
                ("Distal RWS", f"{distal_rws:.1f}%")
            ])
    ...
    def update_profile_display(self):
        ...
        if point_rws_results and stenosis_boundaries:
            ...
            for i, (point_key, pos_key, display_name) in enumerate(zip(point_order, position_keys, display_names)):
            ...
            if positions and rws_values:
                # Plot points
                for i, (pos, rws, name, color) in enumerate(zip(positions, rws_values, point_names, colors)):
                    self.rws_plot.plot([pos], [rws], pen=None, symbol='o', 
                                     symbolSize=12, symbolBrush=color, name=name)
                
                # Connect points with lines
                if len(positions) > 1:
                    self.rws_plot.plot(positions, rws_values, pen=pg.mkPen('gray', width=2, style=Qt.PenStyle.DashLine))
            ...
            if mld_based_rws > 0:
                self.rws_plot.plot([0.5], [mld_based_rws], pen=None, symbol='o', symbolSize=15, symbolBrush='b')
    ...
    def update_segment_display(self):
        ...
        stenosis_rws = self.analysis_results.get('stenosis_rws', {})
        stenosis_boundaries = stenosis_rws.get('stenosis_boundaries', {})
        rws_profile = stenosis_rws.get('rws_profile', [])
        diameter_profiles = stenosis_rws.get('diameter_profiles', {})
        ...
        if point_rws_results:
            ...
            if x_positions and rws_values:
                ...
                for i, (x, rws, name, color) in enumerate(zip(x_positions, rws_values, point_names, colors)):
                    bargraph = pg.BarGraphItem(
                        x=[x], height=[rws], width=0.6,
                        brush=color, name=name
                    )
                    self.segment_plot.addItem(bargraph)
                
                # Add threshold lines
                self.segment_plot.addLine(y=10, pen=pg.mkPen('g', width=1, style=Qt.PenStyle.DashLine))
                self.segment_plot.addLine(y=12, pen=pg.mkPen('y', width=1, style=Qt.PenStyle.DashLine))
                self.segment_plot.addLine(y=15, pen=pg.mkPen('r', width=1, style=Qt.PenStyle.DashLine))
            ...
            if stenosis_rws_val > 0:
                bargraph = pg.BarGraphItem(
                    x=[0], height=[stenosis_rws_val], width=0.6,
                    brush='b', name="Stenosis RWS"
                )
                self.segment_plot.addItem(bargraph)
    ...
    def update_frame_mld_display(self):
        ...
        self.frame_mld_plot.plot(frame_numbers, mld_values, pen=pen, symbol='o', 
                                 symbolSize=8, symbolBrush='b', name='MLD')
        
        # Add mean line
        mean_line = pg.InfiniteLine(pos=mean_mld, angle=0, pen=pg.mkPen('g', width=2, style=Qt.PenStyle.DashLine))
        self.frame_mld_plot.addItem(mean_line)
        
        # Add min/max lines
        min_line = pg.InfiniteLine(pos=min_mld, angle=0, pen=pg.mkPen('r', width=1, style=Qt.PenStyle.DotLine))
        max_line = pg.InfiniteLine(pos=max_mld, angle=0, pen=pg.mkPen('r', width=1, style=Qt.PenStyle.DotLine))
        ...
        self.frame_mld_plot.plot([frame_numbers[min_idx]], [mld_values[min_idx]], 
                                 pen=None, symbol='o', symbolSize=12, 
                                 symbolBrush='r', name=f'Min MLD ({min_mld:.3f}mm)')
        
        self.frame_mld_plot.plot([frame_numbers[max_idx]], [mld_values[max_idx]], 
                                 pen=None, symbol='o', symbolSize=12, 
                                 symbolBrush='g', name=f'Max MLD ({max_mld:.3f}mm)')
