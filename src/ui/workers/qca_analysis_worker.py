...
from src.domain.models.qca_models import (
    QCAAnalysisResult, QCAAnalysisRequest, QCASequentialResult, AnalysisStatus
)
        ...
        try:
            ...
            for frame_num, frame_result in result.results_by_frame.items():
                if frame_result.is_successful:
                    ...
                    self.frame_failed.emit(frame_num, 
                                         frame_result.error_message or "Unknown error")
        ...
        try:
            ...
            def qca_progress(current: int, total: int):
                if not self._is_cancelled:
                    ...
                    self.progress.emit("QCA", percentage, 
                                     f"QCA: Frame {current}/{total}")
            
            # QCA analizini yap
            qca_result = self._qca_service.analyze_sequential(
                self._qca_requests,
                progress_callback=qca_progress
            )
