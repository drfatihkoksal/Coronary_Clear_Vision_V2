...
class AngioPyCalibrationThread(QThread):
    ...
    def run(self):
        try:
            ...
            if (
                "probability" in segmentation_result
                and segmentation_result["probability"] is not None
            ):
            ...
            if len(width_measurements) > (start_offset + end_offset + min_required_measurements):
                ...
                logger.info(
                    f"Excluded {start_offset} points from start and {end_offset} from end, "
                    f"using {len(width_measurements)} measurements"
                )
            else:
                logger.warning(
                    f"Not enough measurements ({len(width_measurements)}) to exclude edge points"
                )
                # Emit warning to user
                self.warning.emit(
                    "Calibration points are too close. Please select points further apart for more accurate calibration."
                )
            ...
            logger.info(
                f"Width measurements from mask: {width_measurements[:5]}... (mean={mean_width:.2f})"
            )
            ...
            if mask is not None:
                ...
                logger.info(
                    f"  - Direct measurements: {catheter_width_measured:.1f} pixels (median of {len(width_measurements)} points)"
                )
                ...
                if catheter_width_pixels < 15:  # Too small for a catheter in 512x512 image
                    logger.warning(
                        f"Catheter width {catheter_width_pixels:.1f} pixels is too small for 512x512 image"
                    )
                    ...
                    if width_measurements:
                        ...
                        logger.info(
                            f"Using mean width from measurements: {catheter_width_pixels:.1f} pixels"
                        )
                    else:
                        # Only use minimum as last resort
                        logger.error(
                            "No width measurements available, calibration may be inaccurate"
                        )
            ...
            logger.info(
                f"Scale factor from 512 to original ({original_width}x{original_height}): {scale_factor:.2f}"
            )
    ...
    def _measure_width_from_mask(
        self, mask: np.ndarray, x: int, y: int, perp_vec: np.ndarray
    ) -> float:
        ...
        width = subpixel_contour_width(
            mask, center_point, perp_vec, method="gradient", is_probability=is_probability
        )

        if width == 0.0:
            # Fallback to moment-based method
            width = subpixel_contour_width(
                mask, center_point, perp_vec, method="moment", is_probability=is_probability
            )
    ...
    def _measure_from_contours(
        self, mask: np.ndarray, p1: np.ndarray, p2: np.ndarray, perp_vec: np.ndarray
    ) -> List[float]:
...
class CalibrationAngioPyWidget(QWidget):
    ...
    def init_ui(self):
        ...
        self.results_label.setStyleSheet(
            "font-size: 11px; color: #666; padding: 5px; background-color: #f5f5f5; border-radius: 3px;"
        )
    ...
    def _on_overlay_changed(self):
        ...
        logger.info(
            f"Overlay settings changed: AngioPy mask={overlay_settings['show_angiopy_mask']}"
        )
    ...
    def toggle_mode(self):
        ...
        if (
            hasattr(self, "overlay_clear_timer")
            and self.overlay_clear_timer
            and self.overlay_clear_timer.isActive()
        ):
    ...
    def perform_calibration(self):
        ...
        self.calibration_thread = AngioPyCalibrationThread(
            self.angiopy_model, image, self.user_points, catheter_size
        )
...
• Measurements: {num_measurements} points{extra_details}
        ...
        logger.info(
            f"Updated calibration values display: {method_text}, {pixels_per_mm:.2f} px/mm, {mm_per_pixel:.5f} mm/px"
        )
