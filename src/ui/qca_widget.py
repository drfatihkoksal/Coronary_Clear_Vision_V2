...
from PyQt6.QtWidgets import (QWidget, QVBoxLayout, QHBoxLayout, QPushButton,
                            QLabel, QGroupBox, QComboBox, QTableWidget,
                            QTableWidgetItem, QCheckBox, QMessageBox,
                            QSplitter, QFileDialog, QTabWidget, QHeaderView,
                            QDialog)
...
class QCAThread(QThread):
    ...
    def __init__(self, qca_analyzer, mask=None, calibration_factor=None, segmentation_result=None,
                 proximal_point=None, distal_point=None, original_image=None,
                 tracked_points=None, use_tracked_centerline=False, global_reference_diameter=None):
        super().__init__()
        ...
        self.global_reference_diameter = global_reference_diameter  # Pre-calculated global reference diameter
    ...
    def run(self):
        try:
            ...
            if self.calibration_factor is not None:
                ...
                logger.info(f"QCAThread: Set calibration factor to {self.calibration_factor:.5f} mm/pixel")
            else:
            ...
            if self.segmentation_result:
                ...
                logger.info(f"[TRACKED DEBUG] QCAThread - use_tracked_centerline: {self.use_tracked_centerline}")
                logger.info(f"[GLOBAL REF DEBUG] QCAThread - global_reference_diameter: {self.global_reference_diameter}")
...
class QCAWidget(QWidget):
    ...
    def init_ui(self):
        ...
        header.setSectionResizeMode(1, header.ResizeMode.ResizeToContents)  # Value column fits content
        ...
        self.summary_table.setHorizontalHeaderLabels(["Frame", "Beat", "Cardiac Phase", "Stenosis %", "MLD (mm)"])
        ...
        summary_header.setSectionResizeMode(2, summary_header.ResizeMode.Stretch)  # Cardiac Phase stretches
        ...
        graph_label.setStyleSheet("font-weight: bold; font-size: 13px; color: #2c3e50; padding: 2px;")
    ...
    def update_sequential_summary(self, all_results: dict):
        ...
        for frame_idx in sorted(all_results.keys()):
            ...
            if result.get('success'):
                ...
                # Debug logging
                logger.info(f"Frame {frame_idx}: cardiac_phase='{result.get('cardiac_phase', '')}', "
    ...
    def on_summary_row_selected(self):
        ...
        if selected_items:
            ...
            if hasattr(main_window, 'viewer_widget') and hasattr(main_window.viewer_widget, 'navigate_to_frame'):
    ...
    def set_main_window(self, main_window):
        ...
        if self.main_window:
            ...
            self.show_segmentation_cb.toggled.connect(self.main_window.toggle_segmentation_visibility)
            ...
            self.clear_all_frames_overlays_btn.clicked.connect(self.main_window.clear_all_frames_overlays)
    ...
    def _save_rws_to_database(self):
        ...
        try:
            ...
            try:
                ...
                patient_id = db_service.set_current_patient("Unknown Patient")
                study_id = db_service.set_current_study()
            ...
            QMessageBox.critical(self, "Database Error", 
                               f"Failed to save RWS data to database:\n{str(e)}")
        
    def analyze_vessel(self, segmentation_result=None, tracked_points=None, use_tracked_centerline=False):
        ...
        logger.info(f"QCA analyzer calibration: {self.qca_analyzer.calibration_factor:.5f} mm/pixel")
        ...
        if self.calibration_factor:
            self.qca_analyzer.calibration_factor = self.calibration_factor
            logger.info(f"analyze_vessel: Set calibration factor to {self.calibration_factor:.5f} mm/pixel")
        else:
            ...
            try:
                ...
                if hasattr(main_window, 'viewer_widget'):
                    ...
                    if current_frame is not None:
                        ...
                        reply = QMessageBox.question(self, "No Segmentation",
    ...
    def _calculate_global_reference_from_sequential_results(self) -> Optional[float]:
        ...
        try:
            logger.info("[GLOBAL REF DEBUG] Starting global reference calculation from sequential results...")
            main_window = self.window()
            ...
            logger.info(f"[GLOBAL REF DEBUG] main_window has sequential_qca_results: {hasattr(main_window, 'sequential_qca_results')}")
            ...
            logger.info(f"[GLOBAL REF DEBUG] sequential_results length: {len(sequential_results) if sequential_results else 0}")
    ...
    def set_calibration(self, calibration_factor: float, details: dict = None):
        ...
        if calibration_factor:
            ...
            self.calib_status_label.setText(f"Calibration: {calibration_factor:.5f} mm/pixel ({catheter_size})")
    ...
    def analysis_finished(self, results):
        ...
        if results.get('diameters') is not None:
            ...
            if valid_diameters:
                logger.info(f"Diameter range: {min(valid_diameters):.2f} - {max(valid_diameters):.2f} pixels")
        logger.info("=" * 80)
    ...
    def update_visualization(self):
        ...
        enabled = any([settings['show_centerline'],
                      settings['show_diameter'],
                      settings['show_stenosis']])
        ...
        try:
            ...
            if hasattr(main_window, 'dicom_parser') and main_window.dicom_parser.has_data():
                ...
                patient_info.update({
                    ...
                    'accession_number': dicom_parser.metadata.get('accession_number', 'Unknown'),
                    'referring_physician': dicom_parser.metadata.get('referring_physician_name', 'Unknown')
                })
        except Exception as e:
    ...
    def perform_rws_analysis(self):
        ...
        try:
            ...
            if not main_window or not hasattr(main_window, 'sequential_qca_results') or not main_window.sequential_qca_results:
                QMessageBox.warning(self, "No Sequential Data",
    ...
    def _perform_standard_rws_analysis(self, main_window):
        ...
        # Validate results
        is_valid, error_msg = QCAWidgetRWS.validate_qca_results_for_rws(main_window.sequential_qca_results)
        if not is_valid:
            QMessageBox.warning(self, "Invalid Data", error_msg)
            return
        
        # Calculate RWS
        rws_results = QCAWidgetRWS.calculate_rws_from_qca_results(main_window.sequential_qca_results)
        ...
        if hasattr(main_window, 'export_rws_to_pdf'):
            dialog.pdf_export_requested.connect(
                lambda: main_window.export_rws_to_pdf(rws_results))
    ...
    def _perform_enhanced_rws_analysis(self, main_window):
        ...
        try:
            ...
            cardiac_phases = None
            if hasattr(main_window, 'viewer_widget') and hasattr(main_window.viewer_widget, 'cardiac_phases'):
            ...
            if not cardiac_phases:
                logger.info("Cardiac phase data not available - Enhanced RWS will use motion detection only")
                # Create empty cardiac phases dict and continue automatically
                cardiac_phases = {'phases': [], 'frame_phases': []}
            
            # Get frame timestamps
            frame_timestamps = None
            if hasattr(main_window, 'viewer_widget') and hasattr(main_window.viewer_widget, 'frame_timestamps'):
            ...
            if frame_timestamps is None:
                QMessageBox.warning(self, "Missing Data",
                                  "Frame timestamps are required for enhanced RWS analysis.")
                return
            ...
            if len(frames) < 2:
                QMessageBox.warning(self, "Insufficient Data",
                                  "Need at least 2 frames for enhanced RWS analysis.")
                return
            ...
            if hasattr(main_window, 'dicom_parser') and main_window.dicom_parser.has_data():
                ...
                if hasattr(main_window, 'ekg_parser') and hasattr(main_window.ekg_parser, 'sampling_rate'):
                ...
                for key, result in main_window.sequential_qca_results.items():
                    try:
                        ...
                        if frame_idx in frame_indices:
                            ...
                            logger.info(f"Frame {frame_idx}: Using existing QCA - {result['percent_stenosis']:.1f}% stenosis, "
                                      f"MLD: {result['mld']:.2f}mm")
                    except (ValueError, TypeError) as e:
                        logger.warning(f"Skipping invalid QCA result key: {key} - {e}")
                
                if len(qca_results_int) == 0:
                    QMessageBox.critical(self, "No QCA Data", 
                                       "No QCA results found for enhanced RWS analysis.\n"
                                       "Please perform QCA analysis first.")
                    return
                
                logger.info(f"Using {len(qca_results_int)} frames with existing QCA results for RWS analysis")
