...
from .models import (
    ...
    CalibrationMeasurement, DicomMetadata, CoronaryVessel, AnalysisType
)
...
class CoronaryAnalysisRepository:
    ...
    def create_patient(self, patient: Patient) -> str:
        ...
        with self.db_manager.get_connection() as conn:
            ...
            """, (
                ...
                patient.gender, patient.medical_record_number, patient.phone,
                patient.email, patient.address, patient.notes
            ))
    ...
    def update_patient(self, patient: Patient) -> bool:
        ...
        with self.db_manager.get_connection() as conn:
            ...
            """, (
                ...
                patient.medical_record_number, patient.phone, patient.email,
                patient.address, patient.notes, patient.updated_at.isoformat(),
                patient.patient_id
            ))
    ...
    def create_study(self, study: Study) -> str:
        ...
        with self.db_manager.get_connection() as conn:
            ...
            """, (
                study.study_id, study.patient_id, study.study_date.isoformat(),
                study.study_description, study.accession_number, 
                study.referring_physician, study.performing_physician,
                study.institution, study.department, study.modality,
                study.study_instance_uid, study.series_instance_uid, study.notes
            ))
    ...
    def create_analysis(self, analysis: Analysis) -> str:
        ...
        with self.db_manager.get_connection() as conn:
            ...
            """, (
                analysis.analysis_id, analysis.study_id, analysis.patient_id,
                analysis.analysis_type.value, analysis.vessel.value,
                analysis.analysis_date.isoformat(), frame_numbers_json,
                analysis.projection_angle, analysis.contrast_volume_ml,
                analysis.radiation_dose_mgy, analysis.operator, analysis.notes,
                analysis.status
            ))
    ...
    def get_analyses_by_study(self, study_id: str) -> List[Analysis]:
        ...
        with self.db_manager.get_connection() as conn:
            rows = conn.execute(
                "SELECT * FROM analyses WHERE study_id = ? ORDER BY analysis_date DESC", 
                (study_id,)
            ).fetchall()
    ...
    def create_rws_data(self, rws_data: RWSData) -> str:
        ...
        with self.db_manager.get_connection() as conn:
            ...
            """, (
                rws_data.rws_id, rws_data.analysis_id, rws_data.rws_percentage,
                rws_data.mld_min_mm, rws_data.mld_max_mm, rws_data.mld_variation_mm,
                rws_data.min_frame_number, rws_data.max_frame_number,
                rws_data.min_mld_position, rws_data.max_mld_position,
                rws_data.cardiac_phase_min, rws_data.cardiac_phase_max,
                rws_data.risk_level, rws_data.clinical_interpretation,
                rws_data.raw_diameter_data, rws_data.outliers_included,
                metadata_json
            ))
    ...
    def get_rws_data_by_analysis(self, analysis_id: str) -> Optional[RWSData]:
        ...
        with self.db_manager.get_connection() as conn:
            row = conn.execute(
                "SELECT * FROM rws_data WHERE analysis_id = ?", 
                (analysis_id,)
            ).fetchone()
    ...
    def create_calibration_data(self, calibration: CalibrationData) -> str:
        ...
        with self.db_manager.get_connection() as conn:
            ...
            """, (
                calibration.calibration_id, calibration.analysis_id,
                calibration.calibration_factor, calibration.catheter_size_french,
                calibration.catheter_diameter_mm, calibration.calibration_method,
                calibration.confidence_score, calibration.point1_x, calibration.point1_y,
                calibration.point2_x, calibration.point2_y, calibration.distance_pixels,
                calibration.distance_mm, metadata_json
            ))
    ...
    def get_calibration_by_analysis(self, analysis_id: str) -> Optional[CalibrationData]:
        ...
        with self.db_manager.get_connection() as conn:
            row = conn.execute(
                "SELECT * FROM calibration_data WHERE analysis_id = ?", 
                (analysis_id,)
            ).fetchone()
    ...
    def create_qca_data(self, qca_data: QCAData) -> str:
        ...
        centerline_json = json.dumps(qca_data.centerline_points) if qca_data.centerline_points else None
        diameter_profile_json = json.dumps(qca_data.diameter_profile) if qca_data.diameter_profile else None
        ...
        with self.db_manager.get_connection() as conn:
            ...
            """, (
                qca_data.qca_id, qca_data.analysis_id, qca_data.frame_number,
                qca_data.vessel_length_mm, qca_data.mean_diameter_mm,
                qca_data.min_diameter_mm, qca_data.max_diameter_mm,
                qca_data.stenosis_percentage, qca_data.stenosis_length_mm,
                qca_data.stenosis_location_mm, qca_data.reference_diameter_mm,
                qca_data.lesion_diameter_mm, centerline_json, diameter_profile_json,
                qca_data.area_stenosis_percentage, qca_data.flow_reserve, metadata_json
            ))
    ...
    def get_qca_data_by_analysis(self, analysis_id: str) -> List[QCAData]:
        ...
        with self.db_manager.get_connection() as conn:
            rows = conn.execute(
                "SELECT * FROM qca_data WHERE analysis_id = ? ORDER BY frame_number", 
                (analysis_id,)
            ).fetchall()
            
            qca_data_list = []
            for row in rows:
                centerline_points = json.loads(row['centerline_points']) if row['centerline_points'] else None
                diameter_profile = json.loads(row['diameter_profile']) if row['diameter_profile'] else None
                metadata = json.loads(row['metadata']) if row['metadata'] else None
