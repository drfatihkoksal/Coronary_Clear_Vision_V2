...
def subpixel_edge_detection(
    ...
    mask_values = ndimage.map_coordinates(
        mask.astype(np.float32), [y_samples, x_samples], order=1, mode="constant", cval=0
    )
...
def subpixel_contour_width(
    ...
    """
    if method == "gradient":
        left_edge, right_edge = subpixel_edge_detection(
            mask, direction, center_point, max_distance=100, is_probability=is_probability
        )
        ...
        # Sample mask values
        mask_profile = ndimage.map_coordinates(
            mask.astype(np.float32), [y_profile, x_profile], order=1, mode="constant", cval=0
        )
...
def subpixel_catheter_diameter(mask: np.ndarray, method: str = "moments") -> float:
    ...
    if method == "moments":
        ...
        contours, _ = cv2.findContours(
            mask.astype(np.uint8), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE  # More precise contour
        )
...
def subpixel_vessel_width_profile(
    mask: np.ndarray, centerline: np.ndarray, smoothing: bool = True, is_probability: bool = None
) -> np.ndarray:
    """
    ...
    for i in range(len(centerline)):
        ...
        if tangent_norm > 0:
            ...
            width = subpixel_contour_width(
                mask, center_point, perp_direction, method="gradient", is_probability=is_probability
            )
            ...
            if width == 0.0 and i % 10 == 0:  # Log every 10th point to avoid spam
                ...
                logger.debug(
                    f"Zero width at point {i}: center={center_point}, perp_dir={perp_direction}"
                )
