...
class RWSAnalysis:
    ...
    def analyze_beat(self, qca_results: Dict[int, Dict], beat_frames: List[int], 
                     calibration_factor: float, patient_info: Optional[Dict] = None) -> Dict:
        """
        ...
        try:
            ...
            logger.info(f"Found {len(valid_results)} valid QCA results out of {len(beat_frames)} beat frames")
            
            if len(valid_results) < 2:
                return {
                    'success': False,
                    'error': 'Insufficient valid QCA data for RWS analysis'
                }
    ...
    def _calculate_rws_from_mld_values(self, diameter_profiles: Dict[int, np.ndarray], qca_results: Dict) -> Dict:
        ...
        logger.info(f"RWS Calculation: MLDmax={max_mld:.2f}mm (Frame {max_mld_frame+1}), MLDmin={min_mld:.2f}mm (Frame {min_mld_frame+1})")
    ...
    def _find_valid_mld(self, mld_candidates: list, proximal_ref: int, distal_ref: int, 
                        diameter_profiles: Dict) -> Optional[int]:
    ...
    def _validate_mld_position(self, mld_idx: int, proximal_ref: Optional[int], 
                               distal_ref: Optional[int]) -> bool:
        ...
        try:
            ...
            for frame_idx, diameters in diameter_profiles.items():
                ...
                if frame_idx == min_frame:
                    ax1.plot(positions, diameters, 'r-', linewidth=2, label=f'Min MLD (Frame {frame_idx + 1})')
                    ...
                    ax1.plot(positions, diameters, 'g-', linewidth=2, label=f'Max MLD (Frame {frame_idx + 1})')
            ...
            if min_frame is not None and min_frame in frame_indices and mld_min_val is not None:
                ...
                ax2.plot(frame_numbers[min_idx], mld_values[min_idx], 'ro', markersize=12, 
            ...
            if max_frame is not None and max_frame in frame_indices and mld_max_val is not None:
                ...
                ax2.plot(frame_numbers[max_idx], mld_values[max_idx], 'go', markersize=12,
                        label=f'Max MLD: {mld_max_val:.2f}mm (Frame {max_frame + 1})')
            
            # Add RWS value as title
            title_text = f'MLD Diameter Variation - RWS = {rws_val}%' if rws_val is not None else 'MLD Diameter Variation - RWS = N/A'
            ...
            if min_frame is not None and min_frame in diameter_profiles:
                ...
                if min_frame in mld_info and mld_info[min_frame]['mld_index'] is not None:
                    ...
                    ax3a.annotate(f'MLD: {diameters[mld_idx]:.2f}mm', 
            ...
            if max_frame is not None and max_frame in diameter_profiles:
                ...
                if max_frame in mld_info and mld_info[max_frame]['mld_index'] is not None:
                    ...
                    ax3b.annotate(f'MLD: {diameters[mld_idx]:.2f}mm', 
                                 xy=(mld_idx, diameters[mld_idx]),
                                 xytext=(10, 10), textcoords='offset points',
    ...
    def generate_pdf_report(self, output_path: str, include_patient_info: bool = True) -> bool:
        ...
        try:
            ...
            with PdfPages(output_path) as pdf:
                ...
                fig_title.text(0.5, 0.9, 'Radial Wall Strain (RWS) Analysis Report', 
                ...
                min_frame_ui = self.rws_results['mld_min_frame'] + 1 if self.rws_results['mld_min_frame'] is not None else 'N/A'
                max_frame_ui = self.rws_results['mld_max_frame'] + 1 if self.rws_results['mld_max_frame'] is not None else 'N/A'
                ...
                report_text += f"  Frames Analyzed: {self.rws_results.get('num_frames_analyzed', 0)}\n"
                ...
                if rws_val is not None and rws_val > 12:
                    report_text += "  ⚠️ HIGH RWS at MLD (>12%): Indicates potential plaque vulnerability\n"
                    report_text += "  Recommendation: Consider further evaluation\n"
                elif rws_val is not None:
                    report_text += "  ✓ NORMAL RWS at MLD (<12%): Indicates stable plaque characteristics\n\n"
                else:
                ...
                if mld_max is not None and mld_min is not None and mld_max > 0:
                    report_text += f"  Calculation: ({mld_max:.2f} - {mld_min:.2f}) / {mld_max:.2f} × 100%\n"
                ...
                fig_title.text(0.1, 0.85, report_text, ha='left', va='top', fontsize=10, 
            ...
            with pd.ExcelWriter(output_path, engine='openpyxl') as writer:
                ...
                summary_data = {
                    ...
                    'Value': [
                        ...
                        self.rws_results.get('mld_max_value', 0) - self.rws_results.get('mld_min_value', 0) if self.rws_results.get('mld_max_value') is not None and self.rws_results.get('mld_min_value') is not None else 'N/A',
                        self.rws_results['mld_min_frame'] + 1 if self.rws_results.get('mld_min_frame') is not None else 'N/A',
                        self.rws_results['mld_max_frame'] + 1 if self.rws_results.get('mld_max_frame') is not None else 'N/A',
                ...
                for frame_idx in sorted(mld_info.keys()):
                    mld_data.append({
                        ...
                        'MLD Position (index)': mld_info[frame_idx]['mld_index'] if mld_info[frame_idx]['mld_index'] is not None else 'N/A'
                    })
    ...
    def get_database_record(self) -> Dict:
        ...
        db_record = {
            ...
            'clinical_interpretation': 'High RWS - Vulnerable' if self.rws_results['rws_at_mld'] > 12 else 'Normal RWS - Stable',
