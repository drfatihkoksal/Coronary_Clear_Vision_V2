    ...
    def compare_methods(self, 
        ...
        try:
            ...
            results['simple'] = {
                ...
                'mean': np.mean(simple_diameters[simple_diameters > 0]) if np.any(simple_diameters > 0) else 0,
                'std': np.std(simple_diameters[simple_diameters > 0]) if np.any(simple_diameters > 0) else 0
            }
        ...
        try:
            matlab_result = self.matlab.measure_diameter_profile(
                mask=mask,
                centerline=centerline
            )
            results['matlab'] = {
        ...
        for method_name, method_result in results.items():
            if isinstance(method_result, dict) and 'diameters' in method_result:
                ...
                if np.any(valid_mask):
                    ...
                    mape = np.mean(np.abs((diameters[valid_mask] - gt[valid_mask]) / gt[valid_mask])) * 100
                    
                    comparison[method_name] = {
                        'mae': float(mae),
                        'rmse': float(rmse),
                        'mape': float(mape),
                        'correlation': float(np.corrcoef(diameters[valid_mask], gt[valid_mask])[0, 1])
                    }
    ...
    def plot_comparison(self, results: Dict, save_path: Optional[str] = None):
        ...
        for method, color in zip(methods, colors):
            if method in results and 'diameters' in results[method]:
                ...
                if len(indices) > 0:
                    plt.plot(indices, diameters[valid_mask], 
