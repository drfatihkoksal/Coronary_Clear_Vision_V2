...
class RWSCalculator:
    ...
    @staticmethod
    def calculate_multi_phase_rws(
        beat_frames: List[int],
        qca_results: Dict[int, Dict],
        selected_frames: Optional[Dict[str, int]] = None,
    ) -> Optional[RWSResult]:
        """
        ...
        if (
            len(beat_frames) < config.rws.min_frames_for_analysis
        ):  # Absolute minimum for meaningful RWS
            logger.error(
                f"Need at least {config.rws.min_frames_for_analysis} frames for RWS calculation, got {len(beat_frames)}"
            )
            return None
        ...
        for frame_idx in beat_frames:
            ...
            if not (config.rws.min_mld_mm <= mld_value <= config.rws.max_mld_mm):
                logger.warning(
                    f"MLD {mld_value:.3f}mm in frame {frame_idx} is outside valid range "
                    f"({config.rws.min_mld_mm}-{config.rws.max_mld_mm}mm)"
                )
                continue

            mld_data.append(
                {"frame": frame_idx, "mld": mld_value, "mld_index": qca_data.get("mld_index")}
            )

            logger.info(
                f"MLD Frame {frame_idx+1}: {mld_value:.3f}mm (index: {qca_data.get('mld_index')})"
            )
        ...
        if (
            len(mld_data) >= 6
        ):  # Need enough frames for IQR method (reduced threshold for less conservative filtering)
            filtered_data = RWSCalculator._remove_mld_outliers(mld_data)
        else:
            logger.info(
                f"Using all {len(mld_data)} frames without outlier removal (too few frames for IQR method)"
            )
        ...
        if max_mld <= min_mld:
            logger.error(
                f"Max MLD ({max_mld:.3f}mm) must be greater than min MLD ({min_mld:.3f}mm)"
            )
            return None
        ...
        try:
            ...
            logger.info(
                f"Step 1: (max - min) = ({max_mld:.3f} - {min_mld:.3f}) = {numerator:.3f}mm"
            )
        ...
        logger.info(
            f"  Min MLD: {min_mld:.3f}mm (Frame {min_data['frame']+1}, {min_phase or 'Unknown phase'})"
        )
        logger.info(
            f"  Max MLD: {max_mld:.3f}mm (Frame {max_data['frame']+1}, {max_phase or 'Unknown phase'})"
        )
    ...
    def _remove_mld_outliers(mld_data: List[Dict]) -> List[Dict]:
        ...
        if removed_outliers:
            ...
            logger.info(
                f"  Protected extremes: min={mld_values[min_idx]:.3f}mm (frame {mld_data[min_idx]['frame']+1}), "
        ...
        if len(filtered_data) < 2:
            logger.warning(
                f"Too few points after Hampel filter ({len(filtered_data)}), using original data"
            )
            return mld_data
    ...
    def validate_result(rws_result: RWSResult) -> bool:
        ...
        if not 0 <= rws_result.rws_percentage <= config.rws.max_rws_percentage:
            logger.error(
                f"Invalid RWS percentage: {rws_result.rws_percentage}% "
    ...
    @staticmethod
    def calculate_multi_beat_average_rws(
        beats_data: List[Dict], qca_results: Dict[int, Dict], use_median: bool = True
    ) -> Optional[RWSResult]:
        """
