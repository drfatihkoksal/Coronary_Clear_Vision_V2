...
class MinimumCostPathGenerator(object):
    ...
    def generate_centerline_from_tracked_points(
        self,
        mask: np.ndarray,
    ...
    def _generate_cost_map(
        self, mask: np.ndarray, smooth_factor: float, use_vessel_guidance: bool
    ) -> np.ndarray:
        """
    ...
    def _create_vessel_guided_cost_map(self, mask: np.ndarray, smooth_factor: float) -> np.ndarray:
        """
    ...
    def _create_distance_cost_map(self, mask: np.ndarray, smooth_factor: float) -> np.ndarray:
        """
    ...
    def _validate_points_bounds(
        self, points: List[Tuple[int, int]], shape: Tuple[int, int]
    ) -> List[Tuple[int, int]]:
        """
    ...
    def _generate_multi_point_path(
        self, cost_map: np.ndarray, points: List[Tuple[int, int]]
    ) -> np.ndarray:
        """
    ...
    def _find_minimum_cost_path(
        self, cost_map: np.ndarray, start: Tuple[int, int], end: Tuple[int, int]
    ) -> List[Tuple[int, int]]:
        """
        ...
        try:
            if SKIMAGE_AVAILABLE:
                ...
                indices, weight = route_through_array(
                    cost_map, start_rc, end_rc, fully_connected=True  # Allow diagonal movement
                )
    ...
    def _smooth_path(self, path: np.ndarray, smooth_factor: float) -> np.ndarray:
        """
    ...
    def visualize_cost_map(self) -> Optional[np.ndarray]:
        ...
        normalized = (
            (self.cost_map - self.cost_map.min())
            / (self.cost_map.max() - self.cost_map.min())
            * 255
        )

        return normalized.astype(np.uint8)

    def get_path_quality_metrics(self, path: np.ndarray) -> Dict[str, float]:
        """
        ...
        if self.last_mask is not None:
            ...
            for point in path:
                ...
                if 0 <= y < self.last_mask.shape[0] and 0 <= x < self.last_mask.shape[1]:
...
def create_centerline_from_tracked_points(
    mask: np.ndarray,
    tracked_points: List[Tuple[float, float]],
    smooth_factor: float = 1.0,
    use_vessel_guidance: bool = True,
