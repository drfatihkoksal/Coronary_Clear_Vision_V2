...
class AngioPySegmentation:
        ...
        **kwargs,
        ...
        try:
            ...
            if not self.model_loaded and not self.model_downloading:
                ...
                try:
                    ...
                    if not cached_path and not self.model_loaded:
                        if progress_callback:
                            progress_callback(
                                "AngioPy model not found. Downloading from Zenodo...", 10
                            )
                        ...
                        def download_progress(current, total):
                            if progress_callback and total > 0:
                                ...
                                progress_callback(
                                    f"Downloading AngioPy model... {mb_current}MB/{mb_total}MB",
                                    percentage,
                                )
                        ...
                        if model_path:
                            ...
                            result["error"] = (
                                "Failed to download AngioPy model. Please check your internet connection."
                            )
                            return result
                finally:
                    self.model_downloading = False
            ...
            if self.model_loaded and self.model is not None:
                ...
                result["error"] = (
                    "AngioPy model not loaded. Please wait for the model to download or check your internet connection."
                )
                result["method"] = "none"
                if progress_callback:
    ...
    def _ai_segmentation(
        ...
        **kwargs,
        ...
        with torch.no_grad():
            ...
            logger.info(
                f"Model output shape: {output.shape}, min: {output.min():.3f}, max: {output.max():.3f}"
            )
            ...
            logger.info(
                f"Probability stats - min: {probability.min():.3f}, max: {probability.max():.3f}, mean: {probability.mean():.3f}"
            )
        ...
        if threshold < 0.5 and np.sum(probability > 0.1) > 100:  # If we have enough non-zero pixels
            percentile_threshold = np.percentile(
                probability[probability > 0.1], 50
            )  # Balanced percentile
        ...
        logger.info(
            f"AI segmentation: probability range [{probability.min():.3f}, {probability.max():.3f}]"
        )
        ...
        if final_mask.shape != original_shape:
            ...
            resized_probability = cv2.resize(
                probability, (original_shape[1], original_shape[0]), interpolation=cv2.INTER_LINEAR
            )
            logger.info(
                f"Resized mask from {final_mask.shape} to {original_shape} with smooth interpolation"
            )
        else:
    ...
    def _preprocess_for_model(
        self, image: np.ndarray, user_points: List[Tuple[int, int]] = None
        ...
        logger.info(
            f"Preprocessing complete - Tensor shape: {tensor.shape}, min: {tensor.min():.3f}, max: {tensor.max():.3f}"
        )
        logger.info(
            f"User points scaled to 512x512: {[(int(p[0]*512/original_shape[1]), int(p[1]*512/original_shape[0])) for p in user_points] if user_points else 'None'}"
        )
    ...
    def _postprocess_mask(
        self,
        mask: np.ndarray,
        user_points: List[Tuple[int, int]] = None,
        ...
        **kwargs,
        ...
        if user_points and len(user_points) >= 2 and original_shape:
            ...
            if use_light_mask_limiting:
                mask = self._limit_mask_to_bounds_light(
                    mask, proximal_distal_points, original_shape
                )
                logger.info(
                    f"Applied LIGHT mask limiting between proximal {user_points[0]} and distal {user_points[-1]} points"
                )
            else:
                mask = self._limit_mask_to_bounds(mask, proximal_distal_points, original_shape)
                logger.info(
                    f"Applied MODERATE mask limiting between proximal {user_points[0]} and distal {user_points[-1]} points"
                )

        return mask

    def _limit_mask_to_bounds(
        self, mask: np.ndarray, user_points: List[Tuple[int, int]], original_shape: Tuple[int, int]
    ) -> np.ndarray:
        """
        ...
        try:
            ...
            logger.info(
                f"MODERATE limiting with reasonable constraints at proximal ({prox_x}, {prox_y}) and distal ({dist_x}, {dist_y})"
            )
            ...
            for i in range(1, num_labels):
                ...
                prox_region = component_mask[
                    max(0, prox_y - search_radius) : min(512, prox_y + search_radius),
                    max(0, prox_x - search_radius) : min(512, prox_x + search_radius),
                ]
                dist_region = component_mask[
                    max(0, dist_y - search_radius) : min(512, dist_y + search_radius),
    ...
    def _limit_mask_to_bounds_light(
        self, mask: np.ndarray, user_points: List[Tuple[int, int]], original_shape: Tuple[int, int]
    ) -> np.ndarray:
        """
        ...
        try:
            ...
            logger.info(
                f"LIGHT limiting with very generous constraints at proximal ({prox_x}, {prox_y}) and distal ({dist_x}, {dist_y})"
            )
            ...
            if length > 3:  # Apply even for very close points
                ...
                logger.warning(
                    "Reference points very close, using very generous elliptical constraint"
                )
    ...
    def _extract_features(self, mask: np.ndarray) -> Tuple[Optional[List], List]:
        ...
        try:
            ...
            if mask.any():
                ...
                logger.info(
                    f"Mask bounds - y: [{np.where(mask)[0].min()}, {np.where(mask)[0].max()}], x: [{np.where(mask)[1].min()}, {np.where(mask)[1].max()}]"
                )
                ...
                if len(centerline_points) > 0:
                    ...
                    ordered_points = self._order_centerline_points_with_reference(
                        centerline_points, user_points
                    )
                    ...
                    if len(centerline) > 10:
                        ...
                        logger.info(
                            f"Centerline sample - Middle: {centerline[len(centerline)//2-2:len(centerline)//2+3].tolist()}"
                        )
                else:
                    centerline = None
            else:
    ...
    def _extract_features_with_angiopy(
        self, mask: np.ndarray, probability: np.ndarray
        ...
        try:
            ...
            if len(skel_points) < 3:
                logger.warning(
                    f"Too few skeleton points ({len(skel_points)}), skipping refinement."
                )
            ...
            subpixel_centerline = self._refine_centerline_to_subpixel(
                integer_centerline, probability
            )

            if subpixel_centerline is not None and len(subpixel_centerline) > 0:
                logger.info(
                    f"Successfully refined centerline to {len(subpixel_centerline)} sub-pixel points."
                )
    ...
    def _refine_centerline_with_distance_transform(
        self, centerline: np.ndarray, mask: np.ndarray
    ) -> np.ndarray:
        """
    ...
    def _order_centerline_points_with_reference(
        self, points: np.ndarray, user_points: List[Tuple[int, int]] = None
    ) -> np.ndarray:
        """
        ...
        if user_points and len(user_points) >= 2:
            ...
            if start_to_distal < start_to_proximal or end_to_proximal < end_to_distal:
                ...
                logger.debug(
                    "Reversed centerline ordering to match proximal->distal reference direction"
                )
            else:
    ...
    def _refine_centerline_to_subpixel(
        self, centerline: np.ndarray, probability_map: np.ndarray
    ) -> np.ndarray:
        """
        ...
        logger.info(
            f"Refined centerline to sub-pixel accuracy. Original points: {len(centerline)}, Refined points: {len(refined_centerline)}"
        )
        return refined_centerline
    ...
    def _apply_modern_centerline_smoothing(self, centerline: np.ndarray) -> np.ndarray:
        ...
        logger.info(
            f"Applied 2025 modern centerline smoothing: preserved {endpoint_preserve} endpoints"
        )
        return smoothed
    ...
    def _remove_reference_point_artifacts(
        self, mask: np.ndarray, user_points: List[Tuple[int, int]]
    ) -> np.ndarray:
        """
        ...
        logger.info(
            f"Reference point artifact removal: {np.sum(mask)} -> {np.sum(cleaned_mask)} pixels"
        )
        return cleaned_mask
    ...
    def _calculate_perpendicular_vector(
        self, centerline: np.ndarray, index: int
    ) -> Optional[np.ndarray]:
        """
    ...
    def calculate_vessel_wall_thickness(self, mask: np.ndarray) -> Dict:
        ...
        try:
            ...
            if len(vessel_distances) > 0:
                ...
                logger.info(
                    f"Vessel wall thickness - Mean: {thickness_stats['mean_thickness']:.2f}px, "
    ...
    def _limit_mask_to_user_region(
        self, mask: np.ndarray, user_points: List[Tuple[int, int]]
    ) -> np.ndarray:
        """
    ...
    def _enforce_centerline_direction(
        self, centerline: np.ndarray, user_points: List[Tuple[int, int]]
    ) -> np.ndarray:
        """
        ...
        try:
            ...
            logger.debug(
                f"Start->Proximal: {start_to_proximal:.2f}, Start->Distal: {start_to_distal:.2f}"
            )
            ...
            correct_orientation = (start_to_proximal < start_to_distal) and (
                end_to_distal < end_to_proximal
            )
            ...
            if not correct_orientation:
                ...
                logger.debug(
                    f"After reversal - Start->Proximal: {new_start_to_proximal:.2f}, End->Distal: {new_end_to_distal:.2f}"
                )
            else:
    ...
    def _interpolate_centerline(
        self, centerline: np.ndarray, target_points: int = 200
    ) -> np.ndarray:
        """
        ...
        try:
            ...
            interp_y = interp1d(
                distances, centerline[:, 0], kind=kind, bounds_error=False, fill_value="extrapolate"
            )
            interp_x = interp1d(
                distances, centerline[:, 1], kind=kind, bounds_error=False, fill_value="extrapolate"
            )
            ...
            logger.info(
                f"Interpolated centerline from {len(centerline)} to {len(interpolated_centerline)} points"
            )
    ...
    def _curvature_resistant_skeleton_processing(
        ...
        try:
            ...
            logger.info(
                f"Curvature-resistant processing: {np.sum(skeleton > 0)} -> {np.sum(final_skeleton > 0)} points"
            )
    ...
    def _remove_small_branches(self, skeleton: np.ndarray, min_length: int = 10) -> np.ndarray:
        ...
        try:
            ...
            for branch_y, branch_x in branch_coords:
                ...
                self._prune_short_paths_from_branch(
                    refined_skeleton, (branch_y, branch_x), min_length
                )
    ...
    def _prune_short_paths_from_branch(
        self, skeleton: np.ndarray, branch_point: tuple, min_length: int
    ):
    ...
    def _trace_path_length(
        self, skeleton: np.ndarray, start_point: tuple, avoid_point: tuple
    ) -> int:
                        ...
                        if (
                            ...
                            for ddy in [-1, 0, 1]:
                                for ddx in [-1, 0, 1]:
                                    ...
                                    if (
                                        0 <= nny < skeleton.shape[0]
                                        and 0 <= nnx < skeleton.shape[1]
    ...
    def _distance_guided_smoothing(
        self, skeleton: np.ndarray, dist_transform: np.ndarray
    ) -> np.ndarray:
    ...
    def _remove_endpoint_artifacts(
        self, skeleton: np.ndarray, mask: np.ndarray, dist_transform: np.ndarray
    ) -> np.ndarray:
            ...
            for ey, ex in endpoint_coords:
                ...
                if len(path) >= 8:  # Need sufficient points to detect curvature
                    ...
                    if self._detect_endpoint_artifact(path, dist_transform):
                        ...
                        logger.info(
                            f"Removed endpoint artifact at ({ey}, {ex}), removed {removal_count} points"
                        )
    ...
    def _trace_endpoint_path(
        self, skeleton: np.ndarray, start_point: tuple, max_length: int = 15
    ) -> list:
    ...
    def _ensure_skeleton_connectivity(self, skeleton: np.ndarray) -> np.ndarray:
        ...
        try:
            ...
            logger.info(
                f"Connectivity: kept largest component ({component_sizes[0][0]} points) out of {num_features} components"
            )
            return result
