...
class QCAReportGenerator:
    ...
    def generate_report(self,
        ...
        try:
            ...
            if angiogram_image is not None:
                ...
                elements.extend(self._create_image_section(angiogram_image, qca_results, with_overlay))
    ...
    def _create_header(self, patient_info: Dict) -> list:
        ...
        # Title
        elements.append(Paragraph("Quantitative Coronary Angiography (QCA) Report",
        ...
        patient_data = [
            ...
            ['Patient Name:', patient_info.get('patient_name', 'N/A'),
             'Accession #:', patient_info.get('accession_number', 'N/A')],
            ['Date of Birth:', patient_info.get('birth_date', 'N/A'),
    ...
    def _create_measurements_table(self, qca_results: Dict) -> list:
        ...
        measurements = [
            ...
            ['Percent Diameter Stenosis', f"{(qca_results.get('percent_stenosis', 0) or 0):.1f}", '%'],
            ['Percent Area Stenosis', f"{(qca_results.get('percent_area_stenosis', 0) or 0):.1f}", '%'],
            ['Minimal Lumen Area (MLA)', f"{(qca_results.get('mla', 0) or 0):.2f}", 'mm²'],
            ['Reference Area', f"{(qca_results.get('reference_area', 0) or 0):.2f}", 'mm²'],
            ['Lesion Length', f"{(qca_results.get('lesion_length', 0) or 0):.1f}" if qca_results.get('lesion_length') is not None else 'N/A', 'mm'],
        ]
    ...
    def _create_stenosis_classification(self, qca_results: Dict) -> list:
        ...
        elements.append(Paragraph(f"<b>Clinical Significance:</b> {significance}", self.styles['Info']))
        elements.append(Spacer(1, 0.3*inch))
    ...
    def _create_image_section(self, angiogram_image: np.ndarray,
                            qca_results: Dict, with_overlay: bool) -> list:
    ...
    def _create_overlay_image(self, image: np.ndarray, qca_results: Dict) -> np.ndarray:
        ...
        if mld_location is not None:
            ...
            cv2.putText(overlay, text, (center[0] + 15, center[1] - 5),
