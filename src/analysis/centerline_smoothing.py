...
def smooth_centerline_endpoints(centerline: np.ndarray, 
                               endpoint_trim: int = 20,
...
def fix_endpoint_artifacts(centerline: np.ndarray, 
                          original: np.ndarray,
                          endpoint_window: int = 10) -> np.ndarray:
    """
    ...
    if start_norm > 0:
        ...
        for i in range(endpoint_window):
            ...
            expected_pos = centerline[endpoint_window] - (endpoint_window - i) * start_direction * start_norm / endpoint_window
            # Orijinal ile blend
    ...
    if end_norm > 0:
        ...
        for i in range(endpoint_window):
            ...
            t = i / endpoint_window
            # Lineer interpolasyon ile düzelt
            expected_pos = centerline[-endpoint_window-1] + (endpoint_window - i) * end_direction * end_norm / endpoint_window
            # Orijinal ile blend
...
def remove_centerline_loops(centerline: np.ndarray, 
...
def adaptive_centerline_smoothing(centerline: np.ndarray,
                                 curvature_threshold: float = 0.5) -> np.ndarray:
    """
...
def preserve_length_smoothing(centerline: np.ndarray,
                            sigma: float = 1.5,
                            endpoint_preserve: int = 20,
                            apply_endpoint_correction: bool = True) -> np.ndarray:
    """
    ...
    if n_points > 2 * endpoint_preserve:
        ...
        middle_y = gaussian_filter1d(centerline[endpoint_preserve:-endpoint_preserve, 0], sigma=sigma)
        middle_x = gaussian_filter1d(centerline[endpoint_preserve:-endpoint_preserve, 1], sigma=sigma)
        ...
        for i in range(endpoint_preserve):
            ...
            # Bitiş tarafı
            end_idx = -(i + 1)
            smoothed[end_idx] = (1 - weight) * centerline[end_idx] + weight * smoothed[-endpoint_preserve-1]
    else:
...
def spline_based_endpoint_correction(centerline: np.ndarray, 
                                   endpoint_window: int = 20) -> np.ndarray:
    """
    ...
    try:
        ...
        if len(inner_segment) > 5:
            ...
            spline_y = UnivariateSpline(t_inner, inner_segment[:, 0], s=len(inner_segment)*0.1, k=3)
            spline_x = UnivariateSpline(t_inner, inner_segment[:, 1], s=len(inner_segment)*0.1, k=3)
            ...
            for j in range(endpoint_window):
                ...
                corrected[j, 0] = (1 - blend_factor) * centerline[j, 0] + blend_factor * extrapolated_y[j]
                corrected[j, 1] = (1 - blend_factor) * centerline[j, 1] + blend_factor * extrapolated_x[j]
        ...
        if len(inner_segment) > 5:
            ...
            for j in range(endpoint_window):
                ...
                corrected[idx, 0] = (1 - blend_factor) * centerline[idx, 0] + blend_factor * extrapolated_y[j]
                corrected[idx, 1] = (1 - blend_factor) * centerline[idx, 1] + blend_factor * extrapolated_x[j]
...
def apply_centerline_smoothing(centerline: np.ndarray,
                             method: str = 'spline',
                             **kwargs) -> np.ndarray:
    """
...
def curvature_adaptive_smoothing(centerline: np.ndarray,
                               curvature_threshold: float = 0.3,
                               endpoint_preserve: int = 20) -> np.ndarray:
    """
