...
class FrameQualityAnalyzer:
    ...
    def calculate_temporal_smoothness_score(self, diameters_sequence: List[float]) -> float:
        ...
        for i in range(1, n - 1):
            ...
            second_derivative = (
                diameters_sequence[i - 1] - 2 * diameters_sequence[i] + diameters_sequence[i + 1]
            )
    ...
    def detect_outlier_frames(
        self, qca_results: Dict[int, Dict], frame_indices: List[int]
        ...
        if len(mld_values) < 3:
            logger.warning(
                f"Insufficient MLD values for outlier detection: {len(mld_values)} values found"
            )
            logger.warning(f"Available MLD values: {list(frame_mld_map.values())}")
            return {idx: 0.0 for idx in frame_indices}, []

        logger.info(
            f"EVT MLD outlier detection starting with {len(mld_values)} values: {list(frame_mld_map.values())}"
        )
        ...
        logger.info(
            f"EVT detected {len(evt_outlier_values)} outlier MLD values: {[f'{v:.3f}mm' for v in evt_outlier_values]}"
        )
        ...
        for idx in frame_indices:
            ...
            is_evt_outlier = any(
                abs(mld - outlier_val) <= tolerance for outlier_val in evt_outlier_values
            )
        ...
        if outlier_frames:
            ...
            logger.info(
                f"  No EVT outlier frames detected - all MLD values within EVT acceptable range"
            )
    ...
    def check_diameter_consistency(self, diameter_profile: np.ndarray) -> Dict[str, float]:
        ...
        if len(diameter_profile) < 10:
            return {"edge_consistency": 1.0, "profile_smoothness": 0.0, "is_consistent": True}

        # Edge vs middle comparison
        edge_size = max(2, len(diameter_profile) // 10)
        edge_avg = (
            np.mean(diameter_profile[:edge_size]) + np.mean(diameter_profile[-edge_size:])
        ) / 2
    ...
    def select_optimal_frames(
        ...
        # Phase code mapping
        phase_mapping = {"early-systole": "s1", "end-systole": "s2", "mid-diastole": "d1"}
        ...
        if "frame_phases" in cardiac_phases:
            for phase_info in cardiac_phases["frame_phases"]:
                ...
                if phase_name:
                    frames = list(range(phase_info["frame_start"], phase_info["frame_end"] + 1))
        ...
        logger.info(
            f"Frame outlier detection: INFORMATIONAL ONLY - frames not filtered by outlier status"
        )
        ...
        for frame_idx in all_frames:
            ...
            if "centerline" in qca_results[frame_idx]:
                ...
                scores["centerline"] = (
                    1.0 - cl_quality["smoothness"] + cl_quality["edge_anomaly_score"]
                )
            ...
            combined_score = (
                self.motion_weight * scores["motion"]
        ...
        for phase_name, candidates in phase_candidates.items():
            ...
            logger.info(
                f"Phase {phase_name}: {len(candidates)} candidate frames, "
            ...
            if not valid_candidates:
                ...
                logger.warning(
                    f"No quality scores available for {phase_name} candidates: {candidates}"
                )
            ...
            if valid_candidates and mld_values_by_frame:
                ...
                if "systole" in phase_name.lower() and global_min_frame in valid_candidates:
                    ...
                    logger.info(
                        f"✅ Using GLOBAL MIN for {phase_name}: Frame {best_frame} ({global_min_mld:.3f}mm)"
                    )
                elif "diastole" in phase_name.lower() and global_max_frame in valid_candidates:
                    best_frame = global_max_frame
                    use_global_extreme = True
                    logger.info(
                        f"✅ Using GLOBAL MAX for {phase_name}: Frame {best_frame} ({global_max_mld:.3f}mm)"
                    )

                if not use_global_extreme:
                    # Fallback to phase-specific extremes
                    candidate_mlds = {
                        f: mld_values_by_frame.get(f, 0)
                    ...
                    if candidate_mlds:
                        ...
                        for frame, mld_val in candidate_mlds.items():
                            ...
                            count = sum(
                                1
                                for other_mld in candidate_mlds.values()
                                if abs(mld_val - other_mld) <= 0.1
                            )  # 0.1mm tolerance
                        ...
                        if "systole" in phase_name.lower():
                            ...
                            min_mld_frames = [
                                f
                                for f, mld in candidate_mlds.items()
                                if abs(mld - min_mld_value) <= 0.01
                            ]  # Tighter tolerance

                        # ALWAYS use the absolute minimum, regardless of frequency
                        best_frame = min(
                            min_mld_frames,
                            key=lambda f: frame_quality_scores.get(f, {}).get(
                                "combined_score", float("inf")
                            ),
                        )

                        freq_info = (
                            f"({len(min_mld_frames)} frames)"
                            if len(min_mld_frames) > 1
                            else "(single occurrence)"
                        )
                        logger.info(
                            f"🔍 TRUE EXTREME MIN selected for {phase_name}: MLD={min_mld_value:.3f}mm {freq_info}"
                        )
                        logger.info(
                            f"  Verifying: Frame {best_frame} actual MLD = {mld_values_by_frame.get(best_frame, 'N/A')}mm"
                        )

                        # Verification: Make sure we got the right frame
                        if abs(mld_values_by_frame.get(best_frame, 0) - min_mld_value) > 0.01:
                            logger.warning(
                                f"  ⚠️ MLD mismatch! Expected {min_mld_value:.3f}mm but got {mld_values_by_frame.get(best_frame, 0):.3f}mm"
                            )
                            ...
                            for f in valid_candidates:
                                if abs(mld_values_by_frame.get(f, 0) - min_mld_value) <= 0.01:
                                    best_frame = f
                                    logger.info(
                                        f"  ✅ Corrected to frame {best_frame} with MLD {mld_values_by_frame.get(f, 0):.3f}mm"
                                    )
                                    break

                    else:  # diastole
                        # For diastole: Find ABSOLUTE maximum MLD
                        sorted_by_mld = sorted(
                            candidate_mlds.items(), key=lambda x: x[1], reverse=True
                        )
                        max_mld_value = sorted_by_mld[0][1]
                        max_mld_frames = [
                            f
                            for f, mld in candidate_mlds.items()
                            if abs(mld - max_mld_value) <= 0.01
                        ]  # Tighter tolerance
                        ...
                        freq_info = (
                            f"({len(max_mld_frames)} frames)"
                            if len(max_mld_frames) > 1
                            else "(single occurrence)"
                        )
                        logger.info(
                            f"🔍 TRUE EXTREME MAX selected for {phase_name}: MLD={max_mld_value:.3f}mm {freq_info}"
                        )
                        logger.info(
                            f"  Verifying: Frame {best_frame} actual MLD = {mld_values_by_frame.get(best_frame, 'N/A')}mm"
                        )
                        ...
                        if abs(mld_values_by_frame.get(best_frame, 0) - max_mld_value) > 0.01:
                            logger.warning(
                                f"  ⚠️ MLD mismatch! Expected {max_mld_value:.3f}mm but got {mld_values_by_frame.get(best_frame, 0):.3f}mm"
                            )
                            # Find the correct frame
                            for f in valid_candidates:
