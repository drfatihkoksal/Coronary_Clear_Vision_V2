...
def measure_vessel_diameter_accurate(
    segmentation_map: np.ndarray, centerline: np.ndarray, max_search_distance: int = 30
) -> np.ndarray:
    """
...
def measure_vessel_diameter_with_edges(
    segmentation_map: np.ndarray, centerline: np.ndarray, max_search_distance: int = 30
) -> Dict[str, np.ndarray]:
    """
    ...
    logger.info(
        f"Map analysis: type={map_info['type']}, range=[{map_info['min']:.3f}, {map_info['max']:.3f}], threshold={map_info['threshold']:.3f}"
    )
...
def _measure_perpendicular_edges_unified(
...
def _measure_perpendicular_edges_mask_constrained(
    segmentation_map: np.ndarray,
    center: np.ndarray,
    perpendicular: np.ndarray,
    max_distance: int,
    ...
    if segmentation_map[cy_int, cx_int] <= threshold:
        logger.debug(
            f"Center point not inside vessel (value: {segmentation_map[cy_int, cx_int]}, threshold: {threshold})"
        )
        return 0.0, 0.0
    ...
    for step in np.arange(step_size, max_distance, step_size):
        ...
        if debug_first_point and step <= 11.0:
            logger.debug(
                f"  Step {step:.1f}: pos=({y:.1f}, {x:.1f}), value={value}, threshold={threshold}"
            )
...
def _measure_perpendicular_edges_subpixel(
    prob_map: np.ndarray, center: np.ndarray, perpendicular: np.ndarray, max_distance: int
) -> Tuple[float, float]:
    """
    ...
    if n_samples < 20:
        n_samples = 20  # Ensure enough samples
    ...
    prob_profile = scipy.ndimage.map_coordinates(
        prob_map,
        [sample_points_y[valid_indices], sample_points_x[valid_indices]],
        order=1,
        mode="constant",
        cval=0.0,
    )
    ...
    window_length = min(
        5, len(prob_profile) if len(prob_profile) % 2 == 1 else len(prob_profile) - 1
    )
    if window_length < 3:
        window_length = 3

    gradient = scipy.signal.savgol_filter(
        prob_profile, window_length=window_length, polyorder=2, deriv=1
    )
...
def _validate_inputs(mask: np.ndarray, centerline: np.ndarray) -> bool:
    ...
    if max_y >= mask.shape[0] or max_x >= mask.shape[1]:
        logger.warning(
            f"Centerline exceeds mask bounds: max_y={max_y}, max_x={max_x}, shape={mask.shape}"
        )
...
def _measure_perpendicular_diameter_exact(
    mask: np.ndarray, center: np.ndarray, perpendicular: np.ndarray, max_distance: int
) -> float:
    """
...
def _measure_perpendicular_edges_exact(
    mask: np.ndarray, center: np.ndarray, perpendicular: np.ndarray, max_distance: int
) -> Tuple[float, float]:
    """
    ...
    for offset in [-0.5, 0, 0.5]:
        ...
        if 0 <= x < mask.shape[1] and 0 <= y < mask.shape[0]:
            ...
            if y_int + 1 < mask.shape[0] and x_int + 1 < mask.shape[1]:
                val = (
                    (1 - y_frac) * (1 - x_frac) * mask[y_int, x_int]
...
def smooth_diameter_profile(
    diameters: np.ndarray, window_size: int = 5, preserve_stenosis: bool = True
) -> np.ndarray:
    """
