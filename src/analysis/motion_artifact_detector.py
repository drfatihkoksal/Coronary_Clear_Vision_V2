...
class MotionArtifactDetector:
    ...
    def detect_motion_artifacts(self, frames: List[np.ndarray], 
                               cardiac_phases: Dict,
                               frame_timestamps: np.ndarray) -> Dict[str, int]:
        """
                ...
                for phase_info in cardiac_phases['frame_phases']:
                    ...
                    if phase_name and phase_name not in selected_frames:
                        ...
                        if phase_frames:
                            ...
                            selected_frames[phase_name] = best_frame
                            logger.info(f"Alternative frame {best_frame} for {phase_name} "
                                      f"(motion score: {motion_scores[best_frame]:.3f})")
    ...
    def _calculate_ssim_region(self, frame1: np.ndarray, frame2: np.ndarray, 
    ...
    def _get_frames_for_phase(self, phase_name: str, cardiac_phases: Dict,
                             frame_timestamps: np.ndarray, total_frames: int) -> List[int]:
    ...
    def _select_best_frame(self, frame_indices: List[int], 
                          motion_scores: np.ndarray) -> int:
        """Select frame with minimal motion artifact from candidates"""
        if not frame_indices:
            return 0
            
        # Get motion scores for candidate frames
        candidate_scores = [(idx, motion_scores[idx]) for idx in frame_indices 
                          if idx < len(motion_scores)]
    ...
    def analyze_motion_quality(self, frames: List[np.ndarray]) -> Dict[str, float]:
        ...
        quality_metrics = {
            ...
            'percent_low_motion': float(np.sum(motion_scores < self.motion_threshold) / len(frames) * 100)
        }
