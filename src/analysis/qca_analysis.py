...
import logging
import traceback
from .vessel_diameter_accurate import measure_vessel_diameter_accurate, measure_vessel_diameter_with_edges, smooth_diameter_profile
from ..utils.subpixel_utils import subpixel_catheter_diameter
from .centerline_smoothing import apply_centerline_smoothing
...
class QCAConstants:
    ...
    MINIMUM_VESSEL_DIAMETER_MM = 0.05  # Minimum allowed vessel diameter in mm (reduced for stenosis detection)
    MINIMUM_VESSEL_DIAMETER_PIXELS = 0.3  # Minimum in pixels to avoid noise (reduced for enhanced sensitivity)
...
class QCAAnalysis:
    ...
    def calibrate_with_catheter(self, 
        ...
        try:
            ...
            validation_result = self._validate_catheter_measurement(pixel_distance, catheter_size, catheter_mm)
            if not validation_result['valid']:
                logger.error(f"Catheter measurement validation failed: {validation_result['reason']}")
                return False
            ...
            if not self._validate_calibration_factor(self.calibration_factor):
                logger.error(f"Calibration factor out of range: {self.calibration_factor:.5f} mm/pixel")
                return False
    ...
    def _measure_catheter_distance_subpixel(self, image: np.ndarray, 
                                          p1: Tuple[int, int], 
    ...
    def _validate_catheter_measurement(self, pixel_distance: float, catheter_size: str, catheter_mm: float) -> Dict:
        """
    ...
    def calculate_global_reference_diameter_from_frames(self, 
                                                     frames_data: List[Dict],
                                                     use_gradient_method: bool = False,
                                                     gradient_method: str = "second_derivative") -> Optional[float]:
        """
        ...
        try:
            ...
            for i, frame_data in enumerate(frames_data):
                try:
                    ...
                    if len(centerline) < 10:
                        logger.warning(f"Frame {i}: Centerline too short ({len(centerline)} points), skipping")
                        continue
            ...
            logger.info(f"Diameter range: {np.min(all_diameters_mm):.2f} - {np.max(all_diameters_mm):.2f} mm")
    ...
    def analyze_from_angiopy(self, 
                            ...
                            global_reference_diameter: float = None) -> Dict:
        """
        ...
        try:
            ...
            logger.debug(f"QCA Analysis - tracked_points length: {len(tracked_points) if tracked_points else 0}")
            ...
            logger.debug(f"analyze_from_angiopy - global_reference_diameter: {global_reference_diameter}")
    ...
    def analyze_multi_frame_vessel(self,
                                 frame_data: List[Dict],
                                 use_gradient_method: bool = False,
                                 gradient_method: str = "second_derivative") -> List[Dict]:
        """
        ...
        try:
            ...
            for i, frame_info in enumerate(frame_data):
                ...
                if len(valid_diameters) > 0:
                    ...
                    logger.info(f"Frame {i}: Collected {len(valid_diameters)} valid diameter measurements")
            ...
            for frame_data_item in frame_diameter_data:
                ...
                logger.info(f"Processing frame {frame_index} with global reference {global_reference_diameter:.2f}mm")
                
                # Calculate stenosis using global reference
                stenosis_results = self._calculate_stenosis(
                    centerline, diameters_px, calibration, 
                    global_reference_diameter=global_reference_diameter
                )
                
                if not stenosis_results['success']:
                    logger.warning(f"Frame {frame_index}: Stenosis calculation failed: {stenosis_results.get('error', 'Unknown')}")
                    continue
                
                # Prepare complete results for this frame
                frame_results = self._prepare_results(
                    centerline, diameters_px, calibration, stenosis_results,
                    diameter_result['left_edges'], diameter_result['right_edges'], 
                ...
                logger.info(f"Frame {frame_index}: {frame_results['percent_stenosis']:.1f}% stenosis (MLD: {frame_results['mld']:.3f}mm)")
    ...
    def analyze_vessel(self, 
        ...
        try:
            ...
            logger.debug(f"analyze_vessel - Received global_reference_diameter: {global_reference_diameter}")
            logger.info("=" * 60)
            ...
            logger.info(f"Centerline bounds - y: [{centerline[:, 0].min():.1f}, {centerline[:, 0].max():.1f}], x: [{centerline[:, 1].min():.1f}, {centerline[:, 1].max():.1f}]")
            ...
            logger.info(f"Centerline sample points: {centerline[:3] if len(centerline) > 3 else centerline}")
            diameter_result = self._measure_diameters(
                mask=mask, 
                centerline=centerline,
            ...
            logger.info(f"Diameter measurements: {len(valid_diameters)} valid of {len(diameters_px)} total")
            logger.info(f"Diameter range: {np.min(valid_diameters):.1f} - {np.max(valid_diameters):.1f} pixels")
            ...
            if self.calibration_factor:
                ...
                logger.debug(f"Diameter range in mm: {np.min(diameters_mm_debug):.2f} - {np.max(diameters_mm_debug):.2f} mm")
                ...
                if np.mean(diameters_mm_debug) > 5.0:
                    logger.warning(f"Mean diameter {np.mean(diameters_mm_debug):.2f}mm is high for coronary arteries (normal: 2-4mm)")
            ...
            if len(valid_indices) == 0:
                return {'success': False, 'error': 'No valid diameter measurements for reference calculation'}
            ...
            logger.info(f"[FRAME REF] Calculated frame-based reference diameter: {frame_reference_diameter:.2f}mm (75th percentile)")
            
            if global_reference_diameter is not None:
                logger.info(f"[GLOBAL REF] Using provided global reference diameter: {global_reference_diameter:.2f}mm")
                final_global_ref = global_reference_diameter
            else:
                logger.info("[GLOBAL REF] No global reference provided - using frame-based reference for stenosis calculation")
            ...
            stenosis_results = self._calculate_stenosis(centerline, diameters_px, calibration, final_global_ref)
            ...
            results = self._prepare_results(
                centerline, diameters_px, calibration, stenosis_results,
                left_edges, right_edges, perpendiculars
            )
    ...
    def _validate_and_fix_coordinates(self, mask: np.ndarray, centerline: np.ndarray) -> np.ndarray:
        ...
        margin = 1.0
        if (max_y >= mask.shape[0] - margin or max_x >= mask.shape[1] - margin or 
            min_y < margin or min_x < margin):
            
            logger.error(f"COORDINATE MISMATCH DETECTED:")
            logger.error(f"Mask dimensions: {mask.shape[0]}x{mask.shape[1]}")
            logger.error(f"Centerline bounds: y[{min_y:.1f}, {max_y:.1f}], x[{min_x:.1f}, {max_x:.1f}]")
            ...
            logger.info(f"Clipped centerline bounds: y[{centerline[:, 0].min():.1f}, {centerline[:, 0].max():.1f}], x[{centerline[:, 1].min():.1f}, {centerline[:, 1].max():.1f}]")
        
        return centerline
    
    def _normalize_centerline_length(self, centerline: np.ndarray, target_points: int = 100) -> np.ndarray:
        """
        ...
        try:
            ...
            if abs(len(centerline) - target_points) <= 5:
                logger.debug(f"Centerline already has {len(centerline)} points, close to target {target_points}")
                return centerline
            ...
            interp_y = interp1d(distances, centerline[:, 0], kind='cubic', 
                               bounds_error=False, fill_value='extrapolate')
            interp_x = interp1d(distances, centerline[:, 1], kind='cubic',
                               bounds_error=False, fill_value='extrapolate')
            
            # Generate normalized centerline
            normalized = np.column_stack([interp_y(target_distances), 
                                         interp_x(target_distances)])
    ...
    def _smooth_with_stenosis_preservation(self, diameters_px: np.ndarray, window_size: int) -> np.ndarray:
        """
        ...
        if len(stenotic_indices) > 0:
            ...
            logger.info(f"STENOSIS PRESERVED: {len(stenotic_indices)} points kept at original values")
            logger.info(f"Stenosis threshold: {stenosis_threshold:.3f}mm (50% of mean {mean_val:.3f}mm)")
    ...
    def _interpolate_centerline(self, centerline: np.ndarray, target_points: int = 50) -> np.ndarray:
        """
    ...
    def _measure_diameters(self, 
                          mask: np.ndarray, 
                          centerline: np.ndarray, 
                          original_image: Optional[np.ndarray] = None,
                          use_gradient_method: bool = False,
                          gradient_method: str = "second_derivative") -> Optional[Dict]:
        """
        ...
        try:
            if use_gradient_method and original_image is not None:
                ...
                from .gradient_diameter_measurement import measure_vessel_diameter_with_edges_gradient
                ...
                logger.info(f"Gradient-based measurement completed with method: {result.get('method', 'unknown')}")
            ...
            if len(non_zero) > 0:
                ...
                logger.info(f"{method_name}-based diameter range: {np.min(non_zero):.1f} - {np.max(non_zero):.1f} pixels")
    ...
    def _smooth_diameters(self, diameters: np.ndarray, smoothing_window: int = 3) -> np.ndarray:
        ...
        logger.debug(f"Diameter smoothing applied: window={smoothing_window}, preserved MLD at index {mld_idx}")
        
        return smoothed
    
    def _calculate_stenosis(self, 
                           centerline: np.ndarray,
        ...
        if len(centerline) > 2 * edge_exclude + 5:
            search_indices = valid_indices[
                ...
                (valid_indices < len(centerline) - edge_exclude)
            ]
        else:
        ...
        logger.info(f"Search diameter range: {np.min(search_diameters):.2f} - {np.max(search_diameters):.2f}mm")
        ...
        if mld > reference_diameter:
            logger.warning(f"MLD ({mld:.3f}mm) > Reference ({reference_diameter:.2f}mm), using reference as upper bound")
            ...
            if mld < 0.1:
                ...
                logger.info(f"Critical stenosis detected: MLD {mld:.3f}mm < 0.1mm, stenosis: {percent_stenosis:.1f}%")
            else:
                percent_stenosis = ((reference_diameter - mld) / reference_diameter * 100)
        ...
        logger.debug(f"Stenosis calculation - MLD: {mld:.3f}mm, Reference: {reference_diameter:.3f}mm")
        logger.debug(f"Stenosis formula: ({reference_diameter:.3f} - {mld:.3f}) / {reference_diameter:.3f} * 100")
    ...
    def _find_reference_diameter(self,
                                diameters_mm: np.ndarray,
                                valid_indices: np.ndarray,
                                mld_idx: int,
                                side: str) -> Tuple[Optional[float], Optional[int]]:
    ...
    def _find_lesion_boundaries(self,
                               diameters_mm: np.ndarray,
                               mld_idx: int,
                               reference_diameter: float) -> Tuple[float, int, int]:
    ...
    def _prepare_results(self,
                        ...
                        perpendiculars: np.ndarray) -> Dict:
