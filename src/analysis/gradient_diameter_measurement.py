...
class GradientDiameterMeasurer:
    ...
    def measure_diameters(self, 
                         ...
                         method: str = "second_derivative") -> Dict[str, np.ndarray]:
        """
    ...
    def _extract_perpendicular_profile(self, 
                                     image: np.ndarray,
                                     center: np.ndarray,
                                     perpendicular: np.ndarray) -> Tuple[Optional[np.ndarray], Optional[np.ndarray]]:
        """
        ...
        distances = np.arange(-half_length, half_length + 1, self.subpixel_sampling)  # Enhanced sub-pixel sampling
        
        sample_y = cy + distances * perpendicular[0]
        sample_x = cx + distances * perpendicular[1]
        
        # Filter out-of-bounds points
        valid_mask = ((sample_y >= 0) & (sample_y < h-1) & 
    ...
    def _detect_edges_first_derivative(self, profile: np.ndarray) -> Tuple[float, float]:
        ...
        for i in range(1, center_idx - 1):
            if (gradient[i] < gradient[i-1] and gradient[i] < gradient[i+1] and 
                gradient[i] < -0.1):  # Threshold for significant edge
    ...
    def _detect_edges_second_derivative(self, profile: np.ndarray) -> Tuple[float, float]:
        ...
        for i in range(center_idx - 1, 0, -1):
            if (second_deriv[i-1] > 0 and second_deriv[i] < 0 and
                abs(first_deriv[i]) > 0.1):  # Significant gradient at zero-crossing
    ...
    def _detect_edges_hybrid(self, profile: np.ndarray) -> Tuple[float, float]:
        ...
        if (total_2nd < self.min_vessel_width or total_2nd > self.max_vessel_width or
            total_2nd < 0.5 * total_1st or total_2nd > 2.0 * total_1st):
            logger.debug(f"Second derivative failed validation, using first derivative: "
    ...
    def _validate_diameter(self, diameter: float, index: int) -> float:
        ...
        if diameter > self.max_vessel_width:
            logger.warning(f"Point {index}: Capping diameter from {diameter:.1f} to {self.max_vessel_width}")
...
def measure_vessel_diameter_gradient(image: np.ndarray, 
                                   centerline: np.ndarray,
                                   method: str = "second_derivative") -> np.ndarray:
    """
...
def measure_vessel_diameter_with_edges_gradient(image: np.ndarray,
                                              centerline: np.ndarray,
                                              method: str = "second_derivative") -> Dict[str, np.ndarray]:
    """
