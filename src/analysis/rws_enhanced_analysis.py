...
class EnhancedRWSAnalysis:
    ...
    def _simple_length_alignment(
        self, diameter_profiles: Dict[str, np.ndarray]
    ) -> Dict[str, np.ndarray]:
    ...
    def analyze_cardiac_cycle(
        ...
        try:
            ...
            if cardiac_phases and cardiac_phases.get("phases"):
                ...
                logger.warning(
                    "No cardiac phase data available, using quality-based selection only"
                )
                selected_frames = self._select_frames_by_quality_only(
                    frames, qca_results, motion_scores_dict
                )

            if len(selected_frames) < 2:
                return {"success": False, "error": "Insufficient frames selected for analysis"}
            ...
            if not stenosis_rws.get("rws_stenosis"):
                return {"success": False, "error": "Failed to calculate stenosis RWS"}
            ...
            logger.info(
                f"Enhanced RWS analysis completed. Stenosis RWS: {stenosis_rws['rws_stenosis']}%"
            )
    ...
    def _coregister_lumen_contours(
        self,
        frames: List[np.ndarray],
        qca_results: Dict[int, Dict],
        ...
        if self.use_tracking_coregistration and frames is not None:
            try:
                ...
                tracking_result = self._coregister_with_tracking(
                    frames, qca_results, selected_frames
                )
    ...
    def _extract_diameter_profiles(
        self, qca_results: Dict[int, Dict], selected_frames: Dict[str, int]
    ) -> Dict[str, np.ndarray]:
    ...
    def _align_by_mld_position(
        self,
        diameter_profile: np.ndarray,
        mld_idx: int,
        reference_length: int,
        reference_mld_idx: int,
    ) -> np.ndarray:
        """
    ...
    def _coregister_diameter_profiles(
        self, diameter_profile: np.ndarray, reference_length: int
    ) -> np.ndarray:
        """
    ...
    def _calculate_stenosis_rws(
        self, selected_frames: Dict[str, int], qca_results: Dict[int, Dict]
    ) -> Dict:
        """
        ...
        if self.enable_coregistration and self.reference_frame is not None:
            ...
            self._perform_coregistration(selected_frames, qca_results)
        else:
        ...
        for phase_name, frame_idx in selected_frames.items():
            if frame_idx in qca_results:
                ...
                if "diameters_mm" in qca_data and qca_data["diameters_mm"] is not None:
                    ...
                    diameter_profile = (
                        np.array(qca_data["diameters_pixels"]) * self.calibration_factor
                    )
        ...
        if reference_frame_idx and reference_frame_idx in qca_results:
            ...
            if "mld_index" in qca_ref or "mld_idx" in qca_ref:
                stenosis_boundaries["mld_point"] = qca_ref.get("mld_index") or qca_ref.get(
                    "mld_idx"
                )
        ...
        for point_name, point_position in anatomical_points.items():
            if point_name == "stenosis":
                ...
                if point_diameters:
                    ...
                    logger.info(
                        f"{point_name.upper()} point uses all {len(point_diameters)} frames (no outlier filtering)"
                    )
                else:
                    max_diameter = min_diameter = 0.0
                    max_phase_name = min_phase_name = "unknown"
            ...
            logger.info(
                f"{point_name.upper()} RWS: {point_rws:.1f}% "
        ...
        logger.info(
            f"Stenosis RWS from MLD values: {stenosis_rws_from_mld}% (MLD_max={max_mld:.3f}mm, MLD_min={min_mld:.3f}mm)"
        )

        logger.info(
            f"Stenosis RWS at MLD: {stenosis_rws_from_mld}% (from MLD: {max_mld:.3f} -> {min_mld:.3f} mm)"
        )
        ...
        return {
            "rws_stenosis": float(
                rws_at_stenosis
            ),  # RWS at stenosis location: (MLD_max - MLD_min) / MLD_min
    ...
    def _analyze_diameter_changes(
        self, selected_frames: Dict[str, int], qca_results: Dict[int, Dict], stenosis_rws: Dict
    ) -> Dict:
        """
    ...
    def _select_frames_by_quality_only(
        self,
        frames: List[np.ndarray],
        qca_results: Dict[int, Dict],
        ...
        """
        logger.info(
            f"Using quality-based frame selection. QCA results available for frames: {list(qca_results.keys())}"
        )

        # Get frames with valid QCA results - handle both int and string keys
        all_keys = list(qca_results.keys())
        logger.info(
            f"QCA result keys types: {[type(k) for k in all_keys[:5]]}"
        )  # Show first 5 key types
        ...
        logger.info(
            f"Quality-based selection: {len(outlier_frames)} outlier frames detected but NOT filtered"
        )
        ...
        if len(frame_diameters) < 3:
            ...
            sorted_by_motion = sorted(
                candidate_frames, key=lambda f: motion_scores.get(f, float("inf"))
            )
        ...
        if diastole_candidates:
            best_diastole = min(
                diastole_candidates, key=lambda f: frame_scores.get(f, float("inf"))
            )
    ...
    def _perform_coregistration(
        self, selected_frames: Dict[str, int], qca_results: Dict
    ) -> Optional[Dict]:
        ...
        try:
            ...
            logger.info(
                f"Successfully co-registered {len(aligned_profiles)} profiles to reference frame {self.reference_frame}"
            )
            return aligned_profiles
        ...
        try:
            ...
            for phase_name, frame_idx in selected_frames.items():
                if frame_idx in qca_results:
                    ...
                    if mask is not None:
                        ...
                        masks.append(
                            np.zeros_like(
                                selected_frame_data[0][:, :, 0]
                                if len(selected_frame_data[0].shape) == 3
                                else selected_frame_data[0]
                            )
                        )
            ...
            if tracking_result and "aligned_profiles" in tracking_result:
                ...
                if "quality_metrics" in tracking_result:
                    ...
                    logger.info(
                        f"  - Tracking başarı oranı: {metrics.get('tracking_success_rate', 0):.2%}"
                    )
                    logger.info(
                        f"  - Ortalama tracking güveni: {metrics.get('average_tracking_confidence', 0):.3f}"
                    )
                    logger.info(
                        f"  - Transform stabilitesi: {metrics.get('transformation_stability', 0):.3f}"
                    )
                    logger.info(
                        f"  - Trajectory düzgünlüğü: {metrics.get('trajectory_smoothness', 0):.3f}"
                    )
