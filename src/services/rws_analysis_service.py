...
import logging
from src.models.rws_models import (RWSAnalysisResult, RWSAnalysisProgress, 
                                  RWSAnalysisRequest, RiskLevel)
...
class RWSAnalysisWorker(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            raw_result = self.qca_analyzer.calculate_rws(
                self.request.qca_results_by_frame,
                self.request.cardiac_phase_info
            )
    ...
    def _emit_progress(self, status: str, percentage: int, 
                      is_complete: bool = False, is_cancelled: bool = False):
        """
    ...
    def _convert_to_model(self, raw_result: Dict) -> RWSAnalysisResult:
        ...
        if not success:
            return RWSAnalysisResult(
                success=False,
                error=raw_result.get('error', 'Unknown error occurred')
            )
    ...
    def _generate_interpretation(self, rws_max: float, risk_level: RiskLevel) -> str:
        ...
        interpretations = {
            ...
            RiskLevel.UNKNOWN: (
                ...
                "Please verify the analysis parameters."
            )
        }
