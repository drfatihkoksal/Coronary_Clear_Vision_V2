...
class HRVCalculatorService(IHRVCalculator):
    ...
    def calculate_hrv(self, rr_intervals: List[RRInterval]) -> HRVMetrics:
        ...
        try:
            ...
            if len(rr_intervals) < self._min_rr_intervals:
                logger.warning(
                    f"Insufficient RR intervals for HRV calculation: {len(rr_intervals)}"
                )
            ...
            logger.info(
                f"HRV calculation completed - Mean RR: {metrics.mean_rr:.1f}ms, RMSSD: {metrics.rmssd:.1f}ms"
            )
            return metrics
    ...
    def _create_empty_metrics(self) -> HRVMetrics:
        ...
        return HRVMetrics(mean_rr=0.0, std_rr=0.0, rmssd=0.0, pnn50=0.0, mean_hr=0.0, std_hr=0.0)
    ...
    def calculate_hrv_indices(self, rr_intervals_ms: np.ndarray) -> Dict[str, float]:
        ...
        try:
            ...
            if len(rr_intervals_ms) > 1:
                ...
                indices["sd2"] = float(
                    np.sqrt(2 * np.var(rr_centered) - 0.5 * np.var(successive_diffs))
                )
