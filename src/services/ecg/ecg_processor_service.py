...
from src.domain.models.ecg_models import ECGSignal
...
class ECGProcessorService(IECGProcessor):
    ...
    def process_signal(self, signal: ECGSignal) -> ECGSignal:
        ...
        try:
            ...
            data = self.apply_bandpass_filter(
                data, self._default_bandpass[0], self._default_bandpass[1], sampling_rate
            )
            ...
            processed_signal = ECGSignal(
                data=data, metadata=signal.metadata, timestamp_offset=signal.timestamp_offset
            )
    ...
    def apply_bandpass_filter(
        self, signal: np.ndarray, low_freq: float, high_freq: float, sampling_rate: float
    ) -> np.ndarray:
        """
        ...
        try:
            ...
            sos = signal.butter(N=2, Wn=[low, high], btype="band", output="sos")  # Filtre derecesi
    ...
    def remove_baseline_wander(self, signal: np.ndarray, sampling_rate: float) -> np.ndarray:
        """
    ...
    def remove_powerline_interference(
        self, signal: np.ndarray, sampling_rate: float, powerline_freq: float = 50.0
    ) -> np.ndarray:
        """
    ...
    def _detect_powerline_interference(self, signal: np.ndarray, sampling_rate: float) -> float:
        """
    ...
    def apply_smoothing(self, signal: np.ndarray, window_size: int = 5) -> np.ndarray:
        """
        ...
        try:
            ...
            smoothed = signal.savgol_filter(signal, window_length=window_size, polyorder=polyorder)
    ...
    def enhance_qrs_complex(self, signal: np.ndarray, sampling_rate: float) -> np.ndarray:
        """
    ...
    def normalize_amplitude(
        self, signal: np.ndarray, target_range: tuple = (-1.0, 1.0)
    ) -> np.ndarray:
        """
