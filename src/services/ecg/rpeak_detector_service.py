...
class RPeakDetectorService(IRPeakDetector):
    ...
    def detect_r_peaks(self, signal: ECGSignal) -> List[RPeak]:
        ...
        try:
            ...
            for i, conf in enumerate(confidences):
                ...
                peak = r_peaks[i]
                r_peaks[i] = RPeak(
                    index=peak.index, time=peak.time, amplitude=peak.amplitude, confidence=conf
                )
    ...
    def calculate_confidence(self, signal: ECGSignal, peaks: List[RPeak]) -> List[float]:
        ...
        try:
            ...
            std_rr / mean_rr if mean_rr > 0 else 1.0
    ...
    def _refine_peak_locations(
        self, original: np.ndarray, peak_indices: List[int], fs: float
    ) -> List[int]:
    ...
    def _post_process_peaks(self, signal: np.ndarray, peaks: List[int], fs: float) -> List[int]:
    ...
    def _create_rpeak_objects(
        self, signal: np.ndarray, indices: List[int], fs: float
    ) -> List[RPeak]:
