...
class CardiacPhaseDetectorService(ICardiacPhaseDetector):
    ...
    def detect_phases(self, signal: ECGSignal, r_peaks: List[RPeak]) -> CardiacPhaseAnalysis:
        """
        ...
        try:
            ...
            analysis = CardiacPhaseAnalysis(cycles=cycles, statistics=statistics, sampling_rate=fs)
    ...
    def get_phase_at_time(
        self, phase_analysis: CardiacPhaseAnalysis, time_s: float
    ) -> Optional[CardiacPhase]:
        """
    ...
    def calculate_phase_statistics(self, cycles: List[CardiacCycle]) -> PhaseStatistics:
        """
        ...
        try:
            if not cycles:
                ...
                return PhaseStatistics(
                    ...
                    d1_offset_ms_mean=0,
                    d1_offset_ms_std=0,
                    systole_duration_ms_mean=0,
                    systole_duration_ms_std=0,
                    diastole_duration_ms_mean=0,
                    diastole_duration_ms_std=0,
                )
    ...
    def _detect_cycle_phases(
        ...
        heart_rate: float,
        next_r_index: Optional[int],
    ) -> Dict[CardiacPhase, CardiacCyclePhase]:
        ...
        phases[CardiacPhase.S1] = CardiacCyclePhase(
            phase=CardiacPhase.S1, index=r_peak.index, time=r_peak.time, phase_name="Early-systole"
        )
        ...
        phases[CardiacPhase.D2] = CardiacCyclePhase(
            phase=CardiacPhase.D2, index=d2_index, time=d2_index / fs, phase_name="End-diastole"
        )

        # S2: End-systole (T dalgası sonu)
        s2_index = self._detect_t_wave_end(
            signal.data, r_peak.index, int(rr_interval_ms * fs / 1000), fs
        )
        ...
        phases[CardiacPhase.S2] = CardiacCyclePhase(
            phase=CardiacPhase.S2, index=s2_index, time=s2_index / fs, phase_name="End-systole"
        )
        ...
        phases[CardiacPhase.D1] = CardiacCyclePhase(
            phase=CardiacPhase.D1, index=d1_index, time=d1_index / fs, phase_name="Mid-diastole"
        )
    ...
    def _detect_t_wave_end(
        self, signal: np.ndarray, r_peak_index: int, rr_interval_samples: int, fs: float
    ) -> Optional[int]:
