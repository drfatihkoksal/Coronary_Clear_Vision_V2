...
from src.domain.models.ecg_models import ECGSignal, ECGQualityMetrics, SignalQuality
...
class ECGQualityAssessorService(IECGQualityAssessor):
    ...
    def __init__(self):
        ...
        self._snr_thresholds = {"excellent": 20.0, "good": 15.0, "fair": 10.0, "poor": 5.0}  # dB
    ...
    def assess_quality(self, signal: ECGSignal) -> ECGQualityMetrics:
        ...
        try:
            ...
            overall_quality = self._determine_overall_quality(
                snr_db, baseline_wander, powerline_interference, motion_artifacts, clipping_ratio
            )
            ...
            logger.info(
                f"Quality assessment completed: {overall_quality.value} (score: {quality_score:.2f})"
            )
            return metrics
    ...
    def _calculate_quality_score(
        ...
        # Ağırlıklı ortalama
        weights = {"snr": 0.4, "baseline": 0.15, "powerline": 0.15, "motion": 0.2, "clipping": 0.1}
    ...
    def _generate_quality_messages(
        self,
        snr_db: float,
        baseline_wander: float,
        powerline_interference: float,
        ...
        if powerline_interference > self._artifact_thresholds["powerline"]:
            messages.append(
                f"Güç hattı girişimi tespit edildi (seviye: {powerline_interference:.2f})"
            )
