...
from src.domain.models.ecg_models import ECGSignal, ECGMetadata, ECGSource
...
class ECGExtractorService(IECGExtractor):
    ...
    def _extract_waveform_sequence(self, ds: Any) -> Optional[ECGSignal]:
        ...
        try:
            ...
            # Örnekleme hızı
            sampling_rate = (
                float(waveform.SamplingFrequency)
            ...
            # Örnek sayısı
            num_samples = (
                int(waveform.NumberOfWaveformSamples)
            ...
            bits = (
                int(waveform.WaveformBitsAllocated)
