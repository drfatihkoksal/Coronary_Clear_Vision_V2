...
class ECGSynchronizerService(IECGSynchronizer):
    ...
    def _create_frame_time_mapping(self, video_duration: float,
                                  frame_rate: float,
                                  time_offset: float) -> Dict[int, float]:
        """
    ...
    def find_ecg_time_for_frame(self, frame_index: int,
                               sync_info: ECGSyncInfo,
                               frame_rate: float) -> float:
        """
    ...
    def find_frame_for_ecg_time(self, ecg_time: float,
                               sync_info: ECGSyncInfo,
                               frame_rate: float) -> int:
        """
    ...
    def calculate_sync_drift(self, ecg_signal: ECGSignal,
                           video_timestamps: np.ndarray) -> float:
        """
        ...
        try:
            ...
            ecg_interval = 1.0 / ecg_signal.metadata.sampling_rate
