...
from src.domain.models.ecg_models import (
    ECGSignal, ECGProcessingRequest, ECGAnalysisResult,
    RPeak, RRInterval, CardiacPhaseAnalysis,
    ECGQualityMetrics, HRVMetrics
)
from src.domain.interfaces.ecg_interfaces import (
    IECGAnalyzer, IECGProcessor, IRPeakDetector,
    ICardiacPhaseDetector, IECGQualityAssessor, IHRVCalculator
)
...
class ECGAnalyzerService(IECGAnalyzer):
        ...
        try:
            ...
            if request.assess_quality:
                ...
                quality_future = self._executor.submit(
                    self._quality_assessor.assess_quality,
                    processed_signal
                )
            ...
            if request.calculate_hrv and rr_intervals:
                ...
                hrv_future = self._executor.submit(
                    self._hrv_calculator.calculate_hrv,
                    rr_intervals
                )
            ...
            if request.detect_phases and r_peaks:
                ...
                phase_future = self._executor.submit(
                    self._phase_detector.detect_phases,
                    processed_signal,
                    r_peaks
                )
            ...
            if quality_future:
                try:
                    ...
                    logger.info(f"Quality assessment completed: {quality_metrics.overall_quality.value}")
    ...
    def analyze_segment(self, signal: ECGSignal, 
                       start_time: float, end_time: float) -> ECGAnalysisResult:
        """
