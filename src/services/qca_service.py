...
class QCAService:
    ...
    def analyze_stenosis(
        ...
        try:
            ...
            reference_diameter = self._calculate_reference_diameter(diameters, reference_segments)
            ...
            stenosis_percent = (reference_diameter - min_diameter) / reference_diameter * 100
    ...
    def _calculate_diameter_profile(self, mask: np.ndarray, centerline: np.ndarray) -> np.ndarray:
        """
    ...
    def _calculate_diameter_at_point(
        self, mask: np.ndarray, centerline: np.ndarray, point_idx: int
    ) -> float:
        """
    ...
    def _calculate_reference_diameter(
        self, diameters: np.ndarray, reference_segments: List[Tuple[int, int]]
    ) -> float:
        """
