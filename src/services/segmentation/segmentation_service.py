...
from src.domain.models.segmentation_models import (
    SegmentationResult, SegmentationRequest, SegmentationValidation,
    VesselFeatures, UserPoint, SegmentationQuality, SegmentationMethod
)
from src.domain.interfaces.segmentation_interfaces import (
    ISegmentationService, ISegmentationEngine, IFeatureExtractor,
    IPostProcessor
)
...
class SegmentationService(ISegmentationService):
    ...
    def segment_vessel(self, request: SegmentationRequest) -> SegmentationResult:
        ...
        try:
            ...
            if request.options.get('post_process', True):
                ...
                result.mask = self._apply_post_processing(
                    result.mask,
                    request.options
                )
            ...
            logger.info(f"Segmentation completed in {result.processing_time_ms:.2f}ms "
    ...
    def refine_segmentation(self,
        ...
        try:
            ...
            if correction_type == 'add':
                # Ekleme düzeltmesi
                refined_mask = self._add_to_mask(
                    current_mask,
                    user_corrections,
                    options
                )
                ...
                # Çıkarma düzeltmesi
                refined_mask = self._remove_from_mask(
                    current_mask,
                    user_corrections,
                    options
                )
            ...
            if options.get('post_process', True):
                refined_mask = self._apply_post_processing(
                    refined_mask,
                    options
                )
    ...
    def validate_segmentation(self, result: SegmentationResult) -> SegmentationValidation:
        ...
        if vessel_pixels < self.MIN_VESSEL_PIXELS:
            validation.add_issue(
                f"Too few vessel pixels detected: {vessel_pixels}"
            )
            validation.add_suggestion(
                "Try selecting more points or adjusting threshold"
            )
        ...
        if result.confidence < self.MIN_CONFIDENCE_THRESHOLD:
            validation.add_suggestion(
                "Low confidence segmentation. Consider manual refinement"
            )
        ...
        if hole_ratio > self.MAX_HOLE_RATIO:
            validation.add_issue(
                f"Too many holes in segmentation: {hole_ratio:.1%}"
            )
            validation.add_suggestion(
                "Enable hole filling in post-processing"
            )
    ...
    def _validate_request(self, request: SegmentationRequest):
        ...
        if (self._engine.requires_user_input and 
            not request.user_points):
            raise ValueError(
                f"{self._engine.method_name} requires user input points"
            )
    ...
    def _apply_post_processing(self,
        ...
        if min_size > 0:
            processed_mask = self._post_processor.remove_small_components(
                processed_mask,
                min_size
            )
