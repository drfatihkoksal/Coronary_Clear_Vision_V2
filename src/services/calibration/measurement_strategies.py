...
from src.domain.models.calibration_models import (
    CatheterMeasurement, MeasurementMethod
)
...
class DistanceTransformMeasurement(ICatheterMeasurement):
    ...
    def measure_width(self,
        ...
        measurement_line = self._calculate_measurement_line(
            segmentation_mask,
            max_loc,
            max_dist
        )
    ...
    def _calculate_measurement_line(self,
                                  mask: np.ndarray,
...
class MinAreaRectMeasurement(ICatheterMeasurement):
    ...
    def _calculate_rect_measurement_line(self, 
                                       rect: Tuple) -> Tuple[Tuple[float, float], Tuple[float, float]]:
        """
...
class SkeletonBasedMeasurement(ICatheterMeasurement):
    ...
    def measure_width(self,
        ...
        for point in skeleton_points[::5]:  # Her 5 noktada bir ölç
            width, line = self._measure_width_at_point(
                segmentation_mask,
                point,
                skeleton
            )
    ...
    def _measure_width_at_point(self,
                              mask: np.ndarray,
                              point: np.ndarray,
                              skeleton: np.ndarray) -> Tuple[float, Optional[Tuple]]:
        """
    ...
    def _find_edge(self, mask: np.ndarray, start: Tuple[float, float], 
