...
from src.domain.interfaces.calibration_interfaces import (
    ICalibrationService,
    ICatheterSegmentation,
    ICatheterMeasurement,
)
...
class CalibrationService(ICalibrationService):
    ...
    def calibrate(self, request: CalibrationRequest) -> CalibrationResult:
        ...
        if request is None:
            ...
            return CalibrationResult(success=False, error_message="Invalid calibration request")

        if request.method is None:
            logger.error("Calibration method not specified")
            return CalibrationResult(
                success=False, error_message="Calibration method must be specified"
            )
        ...
        try:
            ...
            if request.method == CalibrationMethod.CATHETER:
                if request.image is None or request.image.size == 0:
                    return CalibrationResult(
                        success=False, error_message="Image is required for catheter calibration"
                    )
            ...
            return CalibrationResult(
                success=False, error_message=f"Invalid calibration parameters: {str(e)}"
            )
        except Exception as e:
            logger.error(f"Calibration failed: {e}", exc_info=True)
            return CalibrationResult(success=False, error_message=f"Calibration failed: {str(e)}")
    ...
    def validate_calibration(self, result: CalibrationResult) -> CalibrationValidation:
        ...
        if result.factor <= self.MIN_CALIBRATION_FACTOR:
            validation.add_issue(f"Calibration factor too small: {result.factor:.4f} px/mm")
            validation.add_recommendation("Check if the correct units are being used")

        if result.factor >= self.MAX_CALIBRATION_FACTOR:
            validation.add_issue(f"Calibration factor too large: {result.factor:.4f} px/mm")
            validation.add_recommendation("Verify the catheter size or measurement")
    ...
    def calculate_manual_calibration(
        self, points: List[CalibrationPoint], known_distance_mm: float
    ) -> CalibrationResult:
        """
        ...
        if not points or len(points) < 2:
            return CalibrationResult(
                success=False, error_message="At least 2 points required for manual calibration"
            )
        ...
        if known_distance_mm > 50:  # Typical max for coronary reference
            logger.warning(
                f"Large reference distance: {known_distance_mm}mm - verify this is correct"
            )
        ...
        try:
            ...
            return CalibrationResult(
                success=False, error_message="Invalid calibration points provided"
            )
        ...
        try:
            ...
            return CalibrationResult(
                success=False, error_message="Division by zero in calibration calculation"
            )
    ...
    def calculate_catheter_calibration(
        self, measurement: CatheterMeasurement, catheter_size: CatheterSize
    ) -> CalibrationResult:
        """
        ...
        if measurement is None:
            return CalibrationResult(
                success=False, error_message="No catheter measurement provided"
            )

        if catheter_size is None:
            return CalibrationResult(success=False, error_message="No catheter size specified")

        # Validate measurement values
        if not hasattr(measurement, "width_pixels") or measurement.width_pixels <= 0:
            return CalibrationResult(
                success=False, error_message=f"Invalid catheter width measurement"
            )

        # Validate catheter size
        if not hasattr(catheter_size, "diameter_mm") or catheter_size.diameter_mm <= 0:
            return CalibrationResult(
                success=False, error_message=f"Invalid catheter size specification"
            )
        ...
        try:
            ...
            return CalibrationResult(
                success=False, error_message="Failed to calculate calibration factor"
            )
    ...
    def _calibrate_manual(self, request: CalibrationRequest) -> CalibrationResult:
        ...
        if not request.manual_points:
            return CalibrationResult(success=False, error_message="No manual points provided")
        ...
        return self.calculate_manual_calibration(request.manual_points, known_distance_mm)
    ...
    def _calibrate_catheter(self, request: CalibrationRequest) -> CalibrationResult:
        ...
        try:
            ...
            segmentation_mask = self._segmentation.segment_catheter(request.image, request.roi)

            # Kontur çıkarımı
            contours = self._extract_contours(segmentation_mask)
            if not contours:
                return CalibrationResult(
                    success=False, error_message="No catheter contour found in segmentation"
                )
            ...
            measurement = self._measurement.measure_width(segmentation_mask, largest_contour)
            ...
            result = self.calculate_catheter_calibration(measurement, request.catheter_size)

            # Görselleştirme ekle
            if request.options.get("create_visualization", True):
                result.visualization = self._create_visualization(
                    request.image, segmentation_mask, measurement
                )
            ...
            return CalibrationResult(
                success=False, error_message=f"Catheter calibration error: {str(e)}"
            )
    ...
    def _create_visualization(
        self, image: np.ndarray, mask: np.ndarray, measurement: CatheterMeasurement
    ) -> np.ndarray:
        """
        ...
        if measurement.measurement_line:
            ...
            cv2.line(
                vis, (int(start[0]), int(start[1])), (int(end[0]), int(end[1])), (255, 0, 0), 2
            )  # Mavi
