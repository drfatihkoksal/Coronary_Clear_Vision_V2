...
from src.domain.models.qca_models import (
    QCAAnalysisResult, QCAAnalysisRequest, QCASequentialResult,
    VesselMeasurement, StenosisData, AnalysisStatus, CalibrationData
)
from src.domain.interfaces.qca_interfaces import (
    IQCAAnalysisService, IDiameterCalculator, IEdgeDetector, IStenosisDetector
)
...
class QCAAnalysisService(IQCAAnalysisService):
    ...
    def analyze_vessel(self, request: QCAAnalysisRequest) -> QCAAnalysisResult:
        ...
        try:
            ...
            measurements = self._calculate_diameters(
                edges,
                centerline,
                request.calibration
            )
            ...
            logger.info(f"QCA analysis completed for frame {request.frame_number} "
    ...
    def analyze_sequential(self,
                          ...
                          progress_callback: Optional[Callable[[int, int], None]] = None) -> QCASequentialResult:
        """
        ...
        logger.info(f"Sequential analysis completed. "
    ...
    def _validate_segmentation(self, segmentation: Any) -> bool:
        ...
        return (hasattr(segmentation, 'image') and 
                hasattr(segmentation, 'contour') and
                segmentation.image is not None and
                segmentation.contour is not None)
    ...
    def _detect_edges(self, image: np.ndarray, 
        ...
        """
        return self._edge_detector.detect_edges(
            image, 
            initial_contour,
            **options
        )
    
    def _calculate_diameters(self, edges: np.ndarray,
        ...
        """
        return self._diameter_calculator.calculate_diameters(
            edges,
            centerline,
            calibration
        )
    ...
    def _calculate_summary_statistics(self, result: QCASequentialResult):
        ...
        """
        successful_results = [
            r for r in result.results_by_frame.values() 
            if r.is_successful
        ]
        ...
        if all_diameters:
            result.summary_statistics.update({
                ...
                'diameter_variation_percent': (np.max(all_diameters) - np.min(all_diameters)) / np.mean(all_diameters) * 100
            })
        
        # Stenoz istatistikleri
        stenosis_count = sum(1 for r in successful_results if r.has_stenosis)
        result.summary_statistics['stenosis_detection_rate'] = stenosis_count / len(successful_results)
