...
from src.domain.models.dicom_models import (
    DicomStudy, DicomSeries, DicomFrame, DicomLoadRequest,
    DicomLoadResult, DicomPatientInfo, DicomStudyInfo,
    DicomSeriesInfo, DicomImageInfo, DicomProjectionInfo,
    DicomPixelSpacing, DicomWindowLevel, DicomModality,
    ProjectionType
)
from src.domain.interfaces.dicom_interfaces import (
    IDicomLoader, IDicomReader, IDicomWindowLevelProvider,
    IDicomValidator
)
...
class DicomLoaderService(IDicomLoader):
        ...
        try:
            ...
            if self._validator and not self._validator.validate_file_format(request.file_path)[0]:
                return DicomLoadResult(
                    success=False,
                    error_message=f"Invalid DICOM file: {request.file_path}"
                )
    ...
    def _load_single_file(self, request: DicomLoadRequest) -> DicomLoadResult:
        ...
        try:
            ...
            # Görüntü bilgileri
            image_info = self._extract_image_info(dataset)
    ...
    def _load_dicomdir(self, request: DicomLoadRequest) -> DicomLoadResult:
        ...
        try:
            ...
            if not series_list:
                return DicomLoadResult(
                    success=False,
                    error_message="No valid series found in DICOMDIR"
                )
    ...
    def _extract_window_level(self, 
                             dataset: pydicom.Dataset,
                             modality: DicomModality) -> DicomWindowLevel:
    ...
    def _extract_projection_info_from_dataset(self, dataset: pydicom.Dataset) -> Optional[DicomProjectionInfo]:
    ...
    def _load_frames(self, 
                    dataset: pydicom.Dataset,
                    specific_frames: Optional[List[int]] = None) -> List[DicomFrame]:
        """Frame'leri yükle."""
        frames = []
...
class DefaultWindowLevelProvider(IDicomWindowLevelProvider):
    ...
    def get_presets(self, modality: DicomModality) -> Dict[str, DicomWindowLevel]:
        ...
        if modality in [DicomModality.XA, DicomModality.XRF]:
            ...
            return {
                'Default': DicomWindowLevel(128, 256, 'Default')
            }
