...
class DicomExportService(IDicomExporter):
    ...
    def export_frame(self, frame: np.ndarray, output_path: Path, format: str = "png") -> bool:
        ...
        try:
            ...
            if format in ["jpg", "jpeg"]:
                ...
                # PNG için sıkıştırma
                cv2.imwrite(str(output_path), processed_frame, [cv2.IMWRITE_PNG_COMPRESSION, 9])
            else:
                # Diğer formatlar
    ...
    def export_series_as_video(
        ...
        try:
            ...
            writer = cv2.VideoWriter(
                str(output_path), fourcc, fps, (width, height), isColor=len(first_frame.shape) == 3
            )
    ...
    def supported_formats(self) -> List[str]:
        ...
        return (
            self._supported_image_formats + self._supported_video_formats + ["pdf", "html", "json"]
        )
    ...
    def _export_html_report(
        self, study: DicomStudy, output_path: Path, include_images: bool
    ) -> bool:
        """HTML raporu oluştur."""
        try:
            # HTML şablonu
    ...
    def _export_pdf_report(
        ...
        try:
            ...
            study_data = [
                ...
                [
                    "Tarih",
                    str(study.study_info.study_date) if study.study_info.study_date else "N/A",
                ],
