...
class CalibrationConfig(BaseSettings):
    ...
    min_calibration_factor: float = Field(
        default=0.01, description="Minimum calibration factor in px/mm", ge=0.001, le=1.0
    )

    max_calibration_factor: float = Field(
        default=100.0, description="Maximum calibration factor in px/mm", ge=10.0, le=1000.0
    )

    min_catheter_width_pixels: int = Field(
        default=15, description="Minimum catheter width in pixels", ge=5, le=100
    )
    ...
    max_reference_distance_mm: float = Field(
        default=50.0, description="Maximum reference distance in mm", ge=10.0, le=100.0
    )
...
class RWSConfig(BaseSettings):
    ...
    min_mld_mm: float = Field(
        default=0.2, description="Minimum MLD (Minimal Lumen Diameter) in mm", ge=0.1, le=1.0
    )

    max_mld_mm: float = Field(default=6.0, description="Maximum MLD in mm", ge=3.0, le=10.0)

    min_frames_for_analysis: int = Field(
        default=3, description="Minimum frames required for RWS analysis", ge=2, le=10
    )

    max_rws_percentage: float = Field(
        default=200.0, description="Maximum reasonable RWS percentage", ge=100.0, le=500.0
    )
...
class QCAConfig(BaseSettings):
    ...
    min_centerline_points: int = Field(
        default=10, description="Minimum centerline points for analysis", ge=5, le=50
    )

    diameter_smoothing_window: int = Field(
        default=5, description="Window size for diameter smoothing", ge=3, le=15
    )

    max_search_distance_pixels: int = Field(
        default=30, description="Maximum search distance for diameter measurement", ge=10, le=100
    )

    stenosis_threshold_percent: float = Field(
        default=50.0, description="Threshold for significant stenosis", ge=20.0, le=90.0
    )

    global_ref_percentile: float = Field(
        default=75.0, description="Percentile for global reference diameter", ge=50.0, le=95.0
    )
...
class PerformanceConfig(BaseSettings):
    ...
    enable_parallel_processing: bool = Field(
        default=True, description="Enable parallel processing for frame analysis"
    )

    max_worker_threads: int = Field(
        default=4, description="Maximum worker threads for parallel processing", ge=1, le=16
    )

    cache_enabled: bool = Field(default=True, description="Enable caching for analysis results")

    cache_size_mb: int = Field(default=500, description="Maximum cache size in MB", ge=100, le=2000)

    lazy_load_models: bool = Field(default=True, description="Load AI models only when needed")

    progress_update_interval_ms: int = Field(
        default=100, description="Progress bar update interval in milliseconds", ge=50, le=1000
    )
...
class UIConfig(BaseSettings):
    ...
    default_font_size: int = Field(default=13, description="Default font size for UI", ge=8, le=24)

    show_tooltips: bool = Field(default=True, description="Show tooltips in UI")

    auto_save_interval_seconds: int = Field(
        default=300, description="Auto-save interval in seconds (0 to disable)", ge=0, le=3600
    )

    max_recent_files: int = Field(
        default=10, description="Maximum number of recent files to remember", ge=1, le=50
    )

    dark_mode: bool = Field(default=False, description="Enable dark mode theme")
...
class ApplicationConfig(BaseSettings):
    ...
    # General settings
    app_name: str = Field(default="Coronary Clear Vision", description="Application name")

    version: str = Field(default="1.0.0", description="Application version")

    debug_mode: bool = Field(default=False, description="Enable debug mode", env="DEBUG")
    ...
    calibration: CalibrationConfig = Field(
        default_factory=CalibrationConfig, description="Calibration settings"
    )

    rws: RWSConfig = Field(default_factory=RWSConfig, description="RWS analysis settings")

    qca: QCAConfig = Field(default_factory=QCAConfig, description="QCA analysis settings")
