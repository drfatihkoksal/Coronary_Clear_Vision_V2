...
def write_crash_report(exc_type, exc_value, exc_traceback):
    ...
    try:
        ...
        logging.error("Failed to write crash report: %s", e)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Coronary Clear Vision V2 - DICOM Viewer")
    parser.add_argument("dicom_path", nargs="?", help="Path to DICOM file or folder to open")
    parser.add_argument(
        "--folder", "-f", action="store_true", help="Force treating the path as a folder"
    )
    ...
    try:
        ...
        logging.warning("Wayland compatibility check failed: %s", e)
    ...
    try:
        ...
        try:
            ...
            logging.warning("Failed to set application font: %s", e)
        ...
        try:
            ...
            logging.error("Failed to initialize model manager: %s", e)
        ...
        try:
            ...
            # Load DICOM if provided via command line
            if args.dicom_path:
                dicom_path = Path(args.dicom_path)
                if dicom_path.exists():
                    logging.info("Loading DICOM from command line: %s", dicom_path)
                    if dicom_path.is_dir() or args.folder:
                        # Load as folder
                        window.load_dicom_folder_direct(str(dicom_path))
                    else:
                        # Load as file
                        window.load_dicom_file(str(dicom_path))
                else:
                    logging.warning("DICOM path not found: %s", dicom_path)
            ...
            logging.critical("Failed to create main window: %s", e)
